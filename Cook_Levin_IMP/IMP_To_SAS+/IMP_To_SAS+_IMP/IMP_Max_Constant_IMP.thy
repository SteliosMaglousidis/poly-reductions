...
(*
  IMP_Nat_Bijection is now Primities_IMP
...
theory IMP_Max_Constant_IMP
  imports IMP_Max_Constant_Nat
    "IMP_To_IMP-/Primitives_IMP"
    (* "../../IMP/IMP_Nat_Bijection" This theory does not exist... Search for it*)
begin

unbundle IMP_Minus_Com.no_com_syntax

definition atomExp_to_constant_IMP where "atomExp_to_constant_IMP \<equiv>
...
definition atomExp_to_constant_IMP_time where "atomExp_to_constant_IMP_time x \<equiv>
  11 + 2 * fst'_imp_time x"

(*lemma atomExp_to_constant_IMP_time_polynomial: "poly atomExp_to_constant_IMP_time"*)

lemma atomExp_to_constant_IMP_correct:
  "(atomExp_to_constant_IMP, s) \<Rightarrow>\<^bsup>atomExp_to_constant_IMP_time (s ''a'')\<^esup> 
    s(''a'' := 0,
      ''b'' := 0,
      ''c'' := 0,
      ''fst_nat'' := 0,
  ...
  unfolding atomExp_to_constant_IMP_def atomExp_to_constant_tail_def
    atomExp_to_constant_nat_def atomExp_to_constant_IMP_time_def
  by (fastforce 
      intro!: terminates_in_state_intro[OF Seq]
      IMP_fst_nat_correct      
      IMP_snd_nat_correct)+

definition aexp_max_constant_IMP where "aexp_max_constant_IMP \<equiv> 
...
definition aexp_max_constant_IMP_time where "aexp_max_constant_IMP_time x \<equiv>
  38 + nth_nat_IMP_time 1 x + atomExp_to_constant_IMP_time (nth_nat 1 x)
  + nth_nat_IMP_time 2 x +  atomExp_to_constant_IMP_time (nth_nat 2 x)
  + IMP_fst_nat_time (x - 1) + 11" 


lemma aexp_max_constant_IMP_correct:
  "(aexp_max_constant_IMP, s) 
    \<Rightarrow>\<^bsup>aexp_max_constant_IMP_time (s ''a'')\<^esup>
    s(''a'' := 0,
      ''b'' := 0,
      ''c'' := 0,
      ''fst_nat'' := 0,
  ...
  unfolding aexp_max_constant_IMP_def aexp_max_constant_IMP_time_def
  ...
  apply(fastforce simp: numeral_eq_Suc hd_nat_def 
        ...
        atomExp_to_constant_IMP_correct
        nth_nat_IMP_correct
        IMP_max_a_min_b_correct
        IMP_fst_nat_correct)
  (*apply(fastforce simp: numeral_eq_Suc hd_nat_def 
        intro!: terminates_in_time_state_intro[OF Seq]
        atomExp_to_constant_IMP_correct
...
definition add_res_nat_first_part_time where "add_res_nat_first_part_time n s \<equiv>
  (let h = hd_nat s in
    32 + 2 * IMP_fst_nat_time (s - 1) + IMP_fst_nat_time (h - 1)
    + nth_nat_IMP_time (Suc 0) h + nth_nat_IMP_time (Suc (Suc 0)) h
    + nth_nat_IMP_time (Suc (Suc (Suc 0))) h)"
...
lemma add_res_nat_first_part_correct:
  ...
  by (fastforce intro: terminates_in_time_state_intro[OF Seq] 
      terminates_in_state_intro[OF nth_nat_IMP_correct]
      IMP_fst_nat_correct IMP_snd_nat_correct)

definition add_res_nat_second_part where "add_res_nat_second_part \<equiv>
  ''k'' ::= (A (V ''d'')) ;;

  cons_list_IMP [N 7, V ''k'', N 0] ;;   
  cons_IMP (V ''cons'') (N 0) ;;
  ''l'' ::= (A (V ''cons'')) ;;

  cons_list_IMP [N 3, V ''i'', V ''j'', V ''k'', N 0] ;;
  cons_IMP (V ''cons'') (V ''g'') ;;
  ''m'' ::= (A (V ''cons'')) ;;
  
  cons_list_IMP [N 4, V ''i'', V ''j'', V ''nth_nat'', V ''k'', N 0] ;;
  cons_IMP (V ''cons'') (V ''g'') ;;
  ''n'' ::= (A (V ''cons'')) ;;

  cons_list_IMP [N 6, V ''i'', V ''k'', N 0] ;;
  cons_IMP (V ''cons'') (V ''g'') ;;
...
definition add_res_nat_second_part_time where "add_res_nat_second_part_time n s \<equiv>
  (let h = hd_nat s; 
       ...
       l4 = [6, e1, n, 0]
    in 
      18 + cons_list_IMP_time l1 + cons_list_IMP_time l2 
         + cons_list_IMP_time l3 + cons_list_IMP_time l4
         + cons_IMP_time (cons_list l1) 0 + cons_IMP_time (cons_list l2) t
         + cons_IMP_time (cons_list l3) t + cons_IMP_time (cons_list l4) t)"
...
lemma add_res_nat_second_part_correct:  
  ...
  by (fastforce intro: cons_IMP_correct cons_list_IMP_correct)+
...
definition add_res_nat_IMP where "add_res_nat_IMP \<equiv>
  add_res_nat_first_part ;;
  add_res_nat_second_part ;;
  add_res_nat_third_part"

definition add_res_nat_IMP_time where "add_res_nat_IMP_time n s \<equiv>
  add_res_nat_first_part_time n s
  + add_res_nat_second_part_time n s
  + 15
  + zero_variables_time [''a'', ''e'', ''h'', ''k'', ''l'', ''m'', ''n'', ''cons'']"

lemma add_res_nat_IMP_correct:
  "(add_res_nat_IMP, s)
   \<Rightarrow>\<^bsup>add_res_nat_IMP_time (s ''a'') (s ''b'') \<^esup>
      s(''a'' := 0,
        ''b'' := 0,
        ''c'' := 0,
        ''d'' := 0,
...
proof(cases "s ''b''")
  case 0
  show ?thesis
    unfolding add_res_nat_IMP_def add_res_nat_IMP_time_def
...
definition push_con_first_part_time where "push_con_first_part_time c s \<equiv>
  (let con = hd_nat c in
    24 + IMP_fst_nat_time (c - 1) + nth_nat_IMP_time (Suc 0) c
     + nth_nat_IMP_time (Suc (Suc 0)) c  
     + nth_nat_IMP_time (Suc (Suc (Suc 0))) c)"
...
definition push_con_second_part where "push_con_second_part \<equiv>
  ...
  cons_list_IMP [N 1, V ''i'', N 0] ;;
  ''k'' ::= (A (V ''cons'')) ;;
    
  cons_list_IMP [N 2, V ''i'', V ''nth_nat'', N 0] ;;
  ''l'' ::= (A (V ''cons'')) ;;

  cons_list_IMP [N 5, V ''i'', N 0];; 
...
definition push_con_second_part_time where "push_con_second_part_time c s \<equiv>
  (let
    ...
    e3 = nth_nat (Suc (Suc (Suc 0))) c in
      12 + cons_IMP_time 0 0
      + cons_list_IMP_time [1, e2, 0]
      + cons_list_IMP_time [2, e2, e3, 0]
      + cons_list_IMP_time [5, e2, 0])"
...
definition push_con_third_part_time where "push_con_third_part_time c s \<equiv>
  (let
    ...
    l = (
     ...
     (5 ## e2 ## 0)) in
      11 + 18 + cons_IMP_time l s)"

definition push_con_IMP where "push_con_IMP \<equiv>
  push_con_first_part ;;
  push_con_second_part ;;
  push_con_third_part"

definition push_con_IMP_time where "push_con_IMP_time c s \<equiv>
  push_con_first_part_time c s
  + push_con_second_part_time c s
  + push_con_third_part_time c s"

lemma push_con_IMP_correct:
  "(push_con_IMP, s) 
    \<Rightarrow>\<^bsup>push_con_IMP_time (s ''a'') (s ''b'') \<^esup>
     (s(''a'' := 0,
        ''b'' := 0,
        ''c'' := 0,
        ''d'' := 0,
...
proof -
  ...
  thus ?thesis
    apply(elim disjE)
    unfolding push_con_IMP_def push_con_IMP_time_def push_con_nat_def
...
definition max_constant_iteration_first_part_time 
  ...
  (let h = hd_nat s in
    38 + 2 * IMP_fst_nat_time (s - 1) + IMP_fst_nat_time (h - 1)
    + nth_nat_IMP_time (Suc 0) h + nth_nat_IMP_time (Suc (Suc 0)) h
    + nth_nat_IMP_time (Suc (Suc (Suc 0))) h
    + nth_nat_IMP_time (Suc (Suc (Suc (Suc 0)))) h)"
...
definition max_constant_iteration_second_part_time where 
  ...
  (let h = hd_nat s; 
      ...
      (if s = 0 then 1 + 4 
       ...
       (if c = 0 then 1 + 6 + (add_res_nat_IMP_time 0 t) 
       else if c = 1 then 4 + 8 
          + (add_res_nat_IMP_time (aexp_max_constant_tail e1) t)
          + (aexp_max_constant_IMP_time e1)
       else if c = 2 then 7 + 6 + (push_con_IMP_time e1 s) 
       else if c = 3 then 10 + 6 + (push_con_IMP_time e2 s)
       else if c = 4 then 13 + 8 + 11 + (add_res_nat_IMP_time (max e3 e4) t)
       else if c = 5 then 16 + 6 + (push_con_IMP_time e1 s) 
       else if c = 6 then 19 + 6 + (add_res_nat_IMP_time e2 t)
       else 19 + 4)))
...
proof (cases "s' ''a''")
  ...
  then show ?thesis
    ...
    by(fastforce 
        simp: Let_def 
        intro: 
          aexp_max_constant_IMP_correct IMP_max_a_min_b_correct
          add_res_nat_IMP_correct push_con_IMP_correct
        intro!: 
...
definition max_constant_IMP where "max_constant_IMP \<equiv>
  ''max_constant'' ::= (A (N 1)) ;;
...
lemma max_constant_IMP_correct: 
...
(max_constant_IMP, s) 
    \<Rightarrow>\<^bsup>max_constant_time (s ''a'')\<^esup>
       s(''a'' := 0,
         ...
         ''add_res'' := 0,
         ''push_con'' := 0)"
  unfolding max_constant_IMP_def max_constant_time_def 
