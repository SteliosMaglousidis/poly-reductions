theory IMP_To_IMP_Minus_Nat 
  imports  "IMP_To_SAS+_HOL.IMP_To_IMP_Minus"  IMP_Max_Constant_Nat Binary_Operations_Nat 
begin
...
datatype IMPm_IMPmm = Bot IMP_Minus_com |
                      SKIP nat| 
                      Assign vname aexp nat |
                      Seq_0 IMP_com IMP_com nat |
                      Seq_m IMP_com IMP_com nat IMP_Minus_com  |
                      Seq_f IMP_com IMP_com nat IMP_Minus_com IMP_Minus_com  |
                      If_0  vname IMP_com IMP_com nat|
                      If_m  vname IMP_com IMP_com nat IMP_Minus_com  |
                      If_f vname IMP_com IMP_com nat  IMP_Minus_com IMP_Minus_com |
                      While_0 vname IMP_com nat|
                      While_f vname IMP_com nat IMP_Minus_com
...
fun push_on_stack :: "IMP_com \<Rightarrow> nat \<Rightarrow> IMPm_IMPmm list \<Rightarrow> IMPm_IMPmm list" where 
...
fun add_result_to_stack :: "IMP_Minus_com \<Rightarrow> IMPm_IMPmm list \<Rightarrow> IMPm_IMPmm list" where 
...
function IMP_to_IMP_Minus_stack :: "IMPm_IMPmm list \<Rightarrow> IMP_Minus_com" where 
"IMP_to_IMP_Minus_stack (Seq_0 c1 c2 n #stack) = 
  IMP_to_IMP_Minus_stack (push_on_stack c1 n (Seq_0 c1 c2 n #stack))"|
"IMP_to_IMP_Minus_stack (Seq_m c1 c2 n c3 #stack) = 
  IMP_to_IMP_Minus_stack (push_on_stack c2 n (Seq_m c1 c2 n c3 #stack ))"|
"IMP_to_IMP_Minus_stack (If_0 v c1 c2 n # stack) = 
  IMP_to_IMP_Minus_stack (push_on_stack c1 n (If_0 v c1 c2 n #stack ))"|
"IMP_to_IMP_Minus_stack (If_m v c1 c2 n c3 # stack) = 
  IMP_to_IMP_Minus_stack (push_on_stack c2 n (If_m v c1 c2 n c3 #stack ))"|
"IMP_to_IMP_Minus_stack (While_0 v c n # stack) = 
  IMP_to_IMP_Minus_stack (push_on_stack c n (While_0 v c n #stack ))" |
"IMP_to_IMP_Minus_stack (SKIP _ # stack) = 
  IMP_to_IMP_Minus_stack (add_result_to_stack (IMP_Minus_Com.SKIP) stack)" |
"IMP_to_IMP_Minus_stack (Assign v aexp n # stack) = 
  IMP_to_IMP_Minus_stack (add_result_to_stack (assignment_to_binary n v aexp) stack)"|
"IMP_to_IMP_Minus_stack (Seq_f _ _ _ c1 c2 # stack) = 
  IMP_to_IMP_Minus_stack (add_result_to_stack (IMP_Minus_Com.Seq c1 c2) stack)"|
"IMP_to_IMP_Minus_stack (If_f v _ _ n c1 c2 # stack) = 
  IMP_to_IMP_Minus_stack (add_result_to_stack ( IMP_Minus_Com.If (var_bit_list n v) c1  c2) stack)"|
"IMP_to_IMP_Minus_stack (While_f v _ n c # stack) = 
  IMP_to_IMP_Minus_stack (add_result_to_stack (IMP_Minus_Com.While (var_bit_list n v) c) stack)"|
"IMP_to_IMP_Minus_stack (Bot res # stack) =  res"|
"IMP_to_IMP_Minus_stack [] =  com.SKIP"
...
next
  ...
  then show ?case using add_res_less apply auto
    by (metis IMP_To_IMP_Minus_nat.size_stack_var_0 
...
function (domintros) IMP_to_IMP_Minus_stack_nat :: "nat \<Rightarrow> nat" where 
"IMP_to_IMP_Minus_stack_nat s  = ( if s = 0 then 0##0 else let h = hd_nat s ; c = hd_nat h ; t = tl_nat s in
  if c = 3  then  IMP_to_IMP_Minus_stack_nat (push_on_stack_nat (nth_nat (Suc 0) h) (nth_nat (Suc (Suc (Suc 0))) h) s)
else if c = 4 then IMP_to_IMP_Minus_stack_nat (push_on_stack_nat (nth_nat (Suc (Suc 0)) h) (nth_nat (Suc (Suc (Suc 0))) h) s) 
else if c = 6 then IMP_to_IMP_Minus_stack_nat (push_on_stack_nat (nth_nat (Suc (Suc 0)) h) (nth_nat (Suc (Suc (Suc (Suc 0)))) h) s)
else if c = 7 then IMP_to_IMP_Minus_stack_nat (push_on_stack_nat (nth_nat (Suc (Suc (Suc 0))) h) (nth_nat (Suc (Suc (Suc (Suc 0)))) h) s)
else if c = 9 then IMP_to_IMP_Minus_stack_nat (push_on_stack_nat (nth_nat (Suc (Suc 0)) h) (nth_nat (Suc (Suc (Suc 0))) h) s) 
else if c = 1 then IMP_to_IMP_Minus_stack_nat (add_result_to_stack_nat (0##0) t)
else if c = 2 then IMP_to_IMP_Minus_stack_nat (add_result_to_stack_nat (assignment_to_binary_nat (nth_nat (Suc (Suc (Suc 0))) h) (nth_nat (Suc 0) h) (nth_nat (Suc (Suc 0)) h)) t)
else if c = 5 then IMP_to_IMP_Minus_stack_nat (add_result_to_stack_nat (2 ## (nth_nat (Suc (Suc (Suc (Suc 0)))) h) ## (nth_nat (Suc (Suc (Suc (Suc(Suc 0))))) h) ## 0  ) t) 
else if c = 8 then IMP_to_IMP_Minus_stack_nat (add_result_to_stack_nat (3 ## (var_bit_list_nat (nth_nat (Suc (Suc (Suc (Suc 0)))) h)
  (nth_nat (Suc 0) h))
 ## (nth_nat (Suc (Suc (Suc (Suc (Suc 0))))) h) ## (nth_nat (Suc (Suc (Suc (Suc (Suc (Suc 0)))))) h) ## 0  ) t)
else if c = 10 then IMP_to_IMP_Minus_stack_nat (add_result_to_stack_nat (4 ## (var_bit_list_nat (nth_nat (Suc (Suc (Suc 0))) h)
  (nth_nat (Suc 0) h))
...
lemma IMP_To_IMP_Minus_term:
"IMP_to_IMP_Minus_stack_nat_dom (IMPm_IMPmm_list_encode c)"
proof (induct c rule:IMP_to_IMP_Minus_stack.induct)
case (1 c1 c2 n stack)
  then show ?case  using IMP_to_IMP_Minus_stack_nat.domintros[of
"list_encode
...
next
  case 12
  then show ?case by (auto intro: IMP_to_IMP_Minus_stack_nat.domintros
simp add:IMPm_IMPmm_list_encode_def)
qed

lemmas IMP_to_IMP_Minus_stack_nat_simp =
IMP_to_IMP_Minus_stack_nat.psimps[OF IMP_To_IMP_Minus_term]

lemma IMPm_IMPmm_list_encode_0: "(IMPm_IMPmm_list_encode xs = 0) = (xs = [])"
  by (auto simp add: IMPm_IMPmm_list_encode_def list_encode_0)
lemma subtailnat_IMP_to_IMP_Minus_stack:
" IMP_to_IMP_Minus_stack_nat (IMPm_IMPmm_list_encode s)
= comm_encode (IMP_to_IMP_Minus_stack s) "
proof (induct s rule: IMP_to_IMP_Minus_stack.induct )
case (1 c1 c2 n stack)
  then show ?case apply(subst IMP_to_IMP_Minus_stack_nat.psimps)
  using  IMP_To_IMP_Minus_term apply blast
            apply(simp only: Let_def sub_hd head.simps sub_nth nth.simps sub_push_on_stack sub_nth nth.simps
IMPm_IMPmm_list_encode_def list.simps IMPm_IMPmm_encode.simps)
            apply( simp only: sub_push_on_stack  flip:IMPm_IMPmm_encode.simps list.simps IMPm_IMPmm_list_encode_def)
...
lemma IMP_to_IMP_Minus_stack_correct:
"IMP_to_IMP_Minus_stack (push_on_stack c n stack) 
= IMP_to_IMP_Minus_stack (add_result_to_stack (IMP_To_IMP_Minus c n) stack)"
  by (induct c arbitrary:stack) auto

definition IMP_to_IMP_Minus_t:: "IMP_com \<Rightarrow> nat \<Rightarrow> IMP_Minus_com" where
  "IMP_to_IMP_Minus_t c n = 
IMP_to_IMP_Minus_stack (push_on_stack c n [])"

definition IMP_To_IMP_Minus_nat:: "nat \<Rightarrow> nat \<Rightarrow> nat" where
  "IMP_To_IMP_Minus_nat c n = 
IMP_to_IMP_Minus_stack_nat (push_on_stack_nat c n 0)"

lemma IMPm_IMPmm_nil: "0 = IMPm_IMPmm_list_encode []"
  using IMPm_IMPmm_list_encode_def by force

lemma subtailnat_IMP_to_IMP_Minus:
"IMP_To_IMP_Minus_nat (com_encode c) n
= comm_encode (IMP_to_IMP_Minus_t c n)"
  using push_non_empty subtailnat_IMP_to_IMP_Minus_stack
  apply(auto simp only: IMPm_IMPmm_nil IMP_To_IMP_Minus_nat_def IMP_to_IMP_Minus_t_def 
       sub_push_on_stack )
  done

lemma subtail_IMP_to_IMP_Minus:
"IMP_to_IMP_Minus_t c n = 
IMP_To_IMP_Minus c n "
  using  IMP_to_IMP_Minus_stack_correct [of c n "[]"]
  apply (auto simp add:IMP_to_IMP_Minus_t_def)
  done
lemma sub_IMP_To_IMP_Minus:
"IMP_To_IMP_Minus_nat (com_encode c) n = comm_encode (IMP_To_IMP_Minus c n)"
  using subtail_IMP_to_IMP_Minus subtailnat_IMP_to_IMP_Minus by presburger

definition IMP_To_IMP_Minus_tail :: "nat \<Rightarrow> nat \<Rightarrow> nat" where 
"IMP_To_IMP_Minus_tail c n = IMP_To_IMP_Minus_nat c n "

lemma subtail_IMP_To_IMP_Minus:
"IMP_To_IMP_Minus_tail c n = IMP_To_IMP_Minus_nat c n "
  by (simp add: IMP_To_IMP_Minus_tail_def)

definition "IMP_to_IMP_Minus_stack_end_recur s = (
    if s = 0 then 0##0
    else nth_nat 1 (hd_nat s)
)"

definition "IMP_to_IMP_Minus_stack_aux0 c h t = (
...
definition "IMP_to_IMP_Minus_stack_aux1 c h t = (
if c = 8 then (add_result_to_stack_nat (3 ## (var_bit_list_nat (nth_nat 4 h)
  (nth_nat (Suc 0) h))
 ## (nth_nat 5 h) ## (nth_nat 6 h) ## 0  ) t)
else 
 IMP_to_IMP_Minus_stack_aux0 c h t
 )"

definition "IMP_to_IMP_Minus_stack_aux2 c h t = (
if c = 5 then (add_result_to_stack_nat (2 ## (nth_nat 4 h) ## (nth_nat 5 h) 
## 0  ) t)
else IMP_to_IMP_Minus_stack_aux1 c h t)"

definition "IMP_to_IMP_Minus_stack_aux3 c h t = (
if c = 2 then (add_result_to_stack_nat (assignment_to_binary_nat 
   (nth_nat 3 h) (nth_nat 1 h) (nth_nat 2 h)) t)
else IMP_to_IMP_Minus_stack_aux2 c h t)"

definition "IMP_to_IMP_Minus_stack_aux4 c h t = (
if c = 1 then (add_result_to_stack_nat (0##0) t)
else IMP_to_IMP_Minus_stack_aux3 c h t)"

definition "IMP_to_IMP_Minus_stack_aux5 s c h t = (
if c = 9 then (push_on_stack_nat (nth_nat 2 h) (nth_nat 3 h) s)
else IMP_to_IMP_Minus_stack_aux4 c h t)"

definition "IMP_to_IMP_Minus_stack_aux6 s c h t = (
if c = 7 then (push_on_stack_nat (nth_nat 3 h) (nth_nat 4 h) s)
else IMP_to_IMP_Minus_stack_aux5 s c h t)"

definition "IMP_to_IMP_Minus_stack_aux7 s c h t = (
if c = 6 then (push_on_stack_nat (nth_nat 2 h) (nth_nat 4 h) s)
else IMP_to_IMP_Minus_stack_aux6 s c h t)"

definition "IMP_to_IMP_Minus_stack_aux8 s c h t = (
if c = 4 then (push_on_stack_nat (nth_nat 2 h) (nth_nat 3 h) s)
else IMP_to_IMP_Minus_stack_aux7 s c h t)"

definition "IMP_to_IMP_Minus_stack_aux9 s c h t = (
if c = 3 then (push_on_stack_nat (nth_nat 1 h) (nth_nat 3 h) s)
else IMP_to_IMP_Minus_stack_aux8 s c h t)"

lemma IMP_to_IMP_Minus_stack_aux_correct:
assumes "s = IMPm_IMPmm_list_encode c"
shows  "IMP_to_IMP_Minus_stack_nat s = (
    if s = 0 \<or> 10 < (hd_nat (hd_nat s)) \<or> (hd_nat (hd_nat s)) = 0 
    then (IMP_to_IMP_Minus_stack_end_recur s)
    else let h = hd_nat s ; c = hd_nat h ; t = tl_nat s in
    IMP_to_IMP_Minus_stack_nat (IMP_to_IMP_Minus_stack_aux9 s c h t)
  )"
  apply (simp only: IMP_to_IMP_Minus_stack_end_recur_def
  IMP_to_IMP_Minus_stack_aux9_def 
  IMP_to_IMP_Minus_stack_aux8_def
  IMP_to_IMP_Minus_stack_aux7_def
  IMP_to_IMP_Minus_stack_aux6_def
  IMP_to_IMP_Minus_stack_aux5_def
  IMP_to_IMP_Minus_stack_aux4_def
  IMP_to_IMP_Minus_stack_aux3_def
  IMP_to_IMP_Minus_stack_aux2_def
  IMP_to_IMP_Minus_stack_aux1_def
  IMP_to_IMP_Minus_stack_aux0_def Let_def)+
  apply (subst IMP_to_IMP_Minus_stack_nat_simp[of c] assms)+
