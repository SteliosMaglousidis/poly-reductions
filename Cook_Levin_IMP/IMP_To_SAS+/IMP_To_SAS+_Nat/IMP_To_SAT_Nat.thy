theory IMP_To_SAT_Nat 
  imports IMP_To_SAS_Plus_Nat "IMP_To_SAS+_HOL.IMP_To_SAT" SAT_Plan_Base_Nat SAS_Plus_Strips_Nat
          Primitives
begin
...
= imp_to_sat c (poly_of pt) (poly_of p_cer) x"
  ...
  using sublist_IMP_to_SAS_Plus[of " [(''input'', x)]"] sublist_max_input_bits[of " [(''input'', x)]"]
  apply (auto)
  done
...
proof -
  let ?P = "IMP_to_SAS_Plus_list c [(''input'', x)]
              (x + 1 + 2 * 2 ^ poly_of p_cer (Bit_Length.bit_length x)) [(''input'', 0)]
              (t'_pair pt p_cer x)"
  let ?P'= "IMP_to_SAS_Plus c (map_of [(''input'', x)])
              (x + 1 + 2 * 2 ^ poly_of p_cer (Bit_Length.bit_length x)) (map_of [(''input'', 0)])
              (t'_pair pt p_cer x)"
  have "list_problem_to_problem ?P = ?P'" using sublist_IMP_to_SAS_Plus by blast
  ...
  thus "encode_problem_with_operator_interference_exclusion_nat
     (sas_plus_problem_to_strips_problem_nat
       (list_problem_plus_encode
         (prob_with_noop_list
           (IMP_to_SAS_Plus_list c [(''input'', x)]
             (x + 1 + 2 * 2 ^ poly_of p_cer (Bit_Length.bit_length x)) [(''input'', 0)]
             (t'_pair pt p_cer x)))))
     (100 *
