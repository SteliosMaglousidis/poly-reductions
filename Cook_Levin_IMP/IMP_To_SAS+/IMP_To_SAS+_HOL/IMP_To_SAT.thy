...
thm SAS_Plus_to_IMP_correctness

thm IMP_to_SAS_Plus_correctness
...
lemma valid_problem: "is_valid_problem_sas_plus (IMP_to_SAS_Plus c I r G t)"
  using SAS_Plus_Plus_To_SAS_Plus_valid[OF imp_minus_minus_to_sas_plus_valid]
  by (auto simp: IMP_to_SAS_Plus_def Let_def)

lemma while_program_has_model:
  assumes
    "I \<subseteq>\<^sub>m Some \<circ> s1"
    "finite (range s1)"
    (*Mohammad: The next assumption has to be fixed in IMP_to_SAS_Plus_correctness*)
    "Max (range s1) < r"
  ...
  (*Mohammad: there is still a problem with this theorem, namely, the generated formula is still
              ...
              This can only be fixed by generalising IMP_to_SAS_Plus_correctness *)
...
proof-
  obtain plan where plan:
    "length plan \<le> ?b"
    "SAS_Plus_Semantics.is_serial_solution_for_problem (IMP_to_SAS_Plus c I r G t') plan"
    using assms(6) le_trans IMP_to_SAS_Plus_correctness[OF assms(1-5)]
...
proof-
  obtain plan
    where "SAS_Plus_Semantics.is_serial_solution_for_problem (IMP_to_SAS_Plus c I r G t) plan"
    using sas_plus_problem_has_serial_solution_iff_i'[OF valid_problem assms(5)]
    by rule
  thus ?thesis
    using assms
    by(auto intro: SAS_Plus_to_IMP_correctness)
qed
