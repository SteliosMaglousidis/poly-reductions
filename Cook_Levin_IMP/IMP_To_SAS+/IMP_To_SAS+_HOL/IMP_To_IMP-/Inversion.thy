...
text \<open>General definition of procedure for inverting certain functions f :: nat \<Rightarrow> nat TODO: Better description
  ...
  Also some locale to structure refinement process to IMP.
\<close>
...
theory Inversion
  imports Primitives "Landau_Auxiliaries"
    ...
    "HOL-Library.Nat_Bijection" IMP.Call_By_Prefixes
    Triangle_Extensions
begin
...
definition f_inv_bisection'_IMP_while_condition where
  "f_inv_bisection'_IMP_while_condition \<equiv>
...
definition f_inv_bisection'_IMP_loop_body where
  "f_inv_bisection'_IMP_loop_body =
    ...
    invoke_subprogram f_prefix_str f_IMP;;
...
definition f_inv_bisection'_IMP_after_loop where
  "f_inv_bisection'_IMP_after_loop = Com.SKIP"

definition f_inv_bisection'_IMP where
"f_inv_bisection'_IMP \<equiv>
  \<comment> \<open>if L+1 < R\<close>
  f_inv_bisection'_IMP_while_condition;;
  WHILE ''diff''\<noteq>0 DO (
    f_inv_bisection'_IMP_loop_body;;
    f_inv_bisection'_IMP_while_condition
  );;
  f_inv_bisection'_IMP_after_loop"

lemmas f_inv_bisection'_IMP_subprogram_simps =
  f_inv_bisection'_IMP_while_condition_def f_inv_bisection'_IMP_loop_body_def f_inv_bisection'_IMP_after_loop_def
...
(* YEAAAH! *)
lemma f_inv_bisection'_IMP_correct_function_1:
  "(invoke_subprogram p f_inv_bisection'_IMP, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
     s' (add_prefix p f_inv_bisection'_L_str) =
...
proof (induction "f_inv_bisection'_imp_to_HOL_state p s" arbitrary: s s' t rule: f_inv_bisection'_imp.induct)
  ...
  then show ?case
  apply (subst f_inv_bisection'_imp.simps)
  apply (simp only: f_inv_bisection'_IMP_def prefix_simps)
  apply (erule Seq_tE)+
  apply (erule While_tE)
     apply (auto simp add: f_inv_bisection'_imp_state_upd_def prefix_simps Let_def
        f_inv_bisection'_imp_to_HOL_state_def f_inv_bisection'_IMP_subprogram_simps split: if_splits)[]
    apply (erule Seq_tE)+
    apply dest_com_gen


    subgoal (* This seems unproblematic *)
    apply (simp only: f_inv_bisection'_IMP_while_condition_def prefix_simps)
    apply (erule Seq_tE)+
    ...
    subgoal premises p for x s2 y xa s2a ya xb s2b yb xc s2c yc
      ...
      apply (simp only: f_inv_bisection'_IMP_while_condition_def f_inv_bisection'_IMP_loop_body_def prefix_simps)
      apply(erule Seq_tE)+
      apply(all \<open>erule f_IMP_correct[where vars = "f_inv_bisection'_IMP_vars"]\<close>)
...
lemma f_inv_bisection'_IMP_correct_time:
  "(invoke_subprogram p f_inv_bisection'_IMP, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow> t = f_inv_bisection'_imp_time 0 (f_inv_bisection'_imp_to_HOL_state p s)"
proof (induction "f_inv_bisection'_imp_to_HOL_state p s" arbitrary: s s' t rule: f_inv_bisection'_imp.induct)
  case 1
  ...
  apply (erule While_tE_time)
     apply (auto simp add: f_inv_bisection'_imp_state_upd_def prefix_simps Let_def
        f_inv_bisection'_imp_to_HOL_state_def f_inv_bisection'_IMP_subprogram_simps f_inv_bisection'_imp_time_acc split: if_splits)[]
    apply (erule Seq_tE)+
    using "1.hyps"[unfolded f_inv_bisection'_IMP_def] apply -
...
lemma f_inv_bisection'_IMP_correct:
  "\<lbrakk>(invoke_subprogram (p1 @ p2) f_inv_bisection'_IMP, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    \<And>v. v \<in> vars \<Longrightarrow> \<not> (prefix p2 v);
     \<lbrakk>t = (f_inv_bisection'_imp_time 0 (f_inv_bisection'_imp_to_HOL_state (p1 @ p2) s));
      ...
      \<And>v. v \<in> vars \<Longrightarrow> s (add_prefix p1 v) = s' (add_prefix p1 v)\<rbrakk>
     \<Longrightarrow> P\<rbrakk> \<Longrightarrow> P"
  using f_inv_bisection'_IMP_correct_time f_inv_bisection'_IMP_correct_function_1
        IMP_correct_effects
  by auto
...
definition f_inv_bisection_IMP where
"f_inv_bisection_IMP \<equiv>
  (
    ...
    invoke_subprogram f_inv_bisection'_pref f_inv_bisection'_IMP;;
...
lemma f_inv_bisection_IMP_correct_function:
  "(invoke_subprogram p f_inv_bisection_IMP, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
  ...
  apply(simp only: f_inv_bisection_IMP_def com_add_prefix.simps aexp_add_prefix.simps atomExp_add_prefix.simps invoke_subprogram_append)
  apply (erule Seq_tE)+
  apply(all \<open>erule f_inv_bisection'_IMP_correct[where vars = "f_inv_bisection_IMP_vars"]\<close>) (* Probably do not need this here *)
   apply simp
    (* ? *)
   apply auto[]
...
lemma f_inv_bisection_IMP_correct_time:
  "(invoke_subprogram p f_inv_bisection_IMP, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
...
lemma f_inv_bisection_IMP_correct:
  "\<lbrakk>(invoke_subprogram (p1 @ p2) f_inv_bisection_IMP, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    \<And>v. v \<in> vars \<Longrightarrow> \<not> (prefix p2 v);
     \<lbrakk>t = (f_inv_bisection_imp_time 0 (f_inv_bisection_imp_to_HOL_state (p1 @ p2) s));
      ...
      \<And>v. v \<in> vars \<Longrightarrow> s (add_prefix p1 v) = s' (add_prefix p1 v)\<rbrakk>
     \<Longrightarrow> P\<rbrakk> \<Longrightarrow> P"
  using f_inv_bisection_IMP_correct_time f_inv_bisection_IMP_correct_function
        IMP_correct_effects
  by auto
