(*
  ...
  In particular concerning floor_logarithm
...
lemma dlog_Suc_bound: "floor_log (Suc a) \<le> Suc (floor_log a)"
  by (metis Suc_le_D Suc_n_not_le_n floor_log_Suc_zero floor_log_le_iff floor_log_twice nat.simps(3)
    not_less_eq_eq)

(*
  Following are some hammered lemmas to connect logs and floor_log
  Not much thought went into the proofs
*)
lemma dlog_log2_bound: "n > 0 \<Longrightarrow> floor_log n \<le> log 2 n"
  by (simp add: le_log2_of_power floor_log_exp2_le)

lemma log2_floor_dlog_eq:
  assumes "n > 0"
  shows "floor_log n = nat \<lfloor>log 2 n\<rfloor>"
  apply (rule floor_log_eqI)
  using assms apply assumption
  using assms floor_log_altdef floor_log_exp2_le apply auto[1]
  using assms bot_nat_0.not_eq_extremum floor_log_altdef floor_log_exp2_gt by presburger

(* Experiment: Set up real_asymp for floor_log. Makes everything below, but the last lemma superflouus  *)
lemma dlog_express_as_real[real_asymp_nat_reify]:
  "real (floor_log n) = Multiseries_Expansion.rfloor (log 2 (max (real n) 1))"
  using log2_floor_dlog_eq rfloor_def by (cases n) auto

corollary log2_dlog_bound: "n > 0 \<Longrightarrow> floor_log n \<ge> nat \<lfloor>log 2 n\<rfloor>"
  using log2_floor_dlog_eq by force

lemma log2_floor_dlog_bound:
  assumes "n > 0"
  shows "1 + floor_log n \<ge> nat \<lceil>log 2 n\<rceil>"
  by (metis assms floor_log_exp2_ge log2_of_power_le mult_Suc_right mult_numeral_1 nat_ceiling_le_eq
    numerals(1) power.simps(2))

lemma log2_ceil_in_Odlog': "(\<lambda>n. \<lceil>log 2 n\<rceil>) \<in> O(\<lambda>n. 1 + floor_log n)"
  by real_asymp
  (*
  ...
  *)
lemma pos_const_in_odlog: "const > 0 \<Longrightarrow>(\<lambda>_. const::real) \<in> o(floor_log)"
  by real_asymp
  (*
  ...
  subgoal for c
    ...
    proof-
      ...
      have "c * floor_log (2 ^ nat \<lceil>(const/c)\<rceil>) \<ge> const"
      ...
      ultimately show ?thesis
        ...
        by (metis floor_log_le_iff linorder_not_less log_exp mult.commute of_nat_le_iff order_less_le_trans pos_less_divide_eq)
    qed
    done
  done
  *)

lemma const_in_odlog: "(\<lambda>_. const::real) \<in> o(floor_log)"
  by real_asymp
...
lemma dlog_plus1_theta: "\<Theta>(\<lambda>n. floor_log n + 1) = \<Theta>(floor_log)"
  using const_in_odlog by auto

lemma log2_ceil_in_Odlog: "(\<lambda>n. \<lceil>log 2 n\<rceil>) \<in> O(floor_log)"
  by real_asymp
(*
proof-
  have "(\<lambda>n. floor_log n) \<in> \<Theta>(\<lambda>n. floor_log n + 1)"
    using dlog_plus1_theta by auto
  hence 1: "(\<lambda>n. floor_log n + 1) \<in> O(\<lambda>n. floor_log n)"
    by (meson landau_o.big.bigtheta_trans2' landau_o.big_refl)
  have 2: "(\<lambda>n. \<lceil>log 2 n\<rceil>) \<in> O(\<lambda>n. floor_log n + 1)"
    apply (rule bigoI[of _ 1])
    apply auto
...
lemma dlog_in_Olog2: "floor_log \<in> O(log 2)"
  by real_asymp
(*by (rule bigoI[of _ 1]) (simp add: log_altdef)*)

lemma floor_log2_in_Odlog: "(\<lambda>n. \<lfloor>log 2 n\<rfloor>) \<in> O(floor_log)"
  by real_asymp
(*
  apply (rule bigoI[of _ "1"])
...
lemma dlog_in_Oln: "floor_log \<in> O(ln)"
  by real_asymp
  (* using dlog_in_Olog2 landau_o.big_trans log2_in_Oln by blast*)

lemma log2_in_Odlog: "log 2 \<in> O(floor_log)"
  by real_asymp
(*
proof-
...
corollary dlog_in_\<Omega>log2: "floor_log \<in> \<Omega>(log 2)"
  by real_asymp (* using bigo_imp_bigomega log2_in_Odlog by blast*)
corollary dlog_in_\<Theta>log2: "floor_log \<in> \<Theta>(log 2)"
  by real_asymp
  (*
  using dlog_in_\<Omega>log2 dlog_in_Olog2 by auto*)
corollary dlog_in_\<Theta>ln: "floor_log \<in> \<Theta>(ln)"
  by real_asymp
  (*
  using dlog_in_\<Theta>log2 log2_\<Theta>_ln' by blast*)

(* The result of the whole diversion *)
corollary dlog_\<Theta>_ln: "\<Theta>(floor_log) = \<Theta>(ln)"
