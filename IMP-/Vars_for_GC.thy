...
lemma subst_sound:
  "\<lbrakk> (c,s) \<Rightarrow>\<^sub>G\<^bsup>z\<^esup> t; s = s' o m on S; set (vars c) \<subseteq> S; inj_on m S \<rbrakk>
    \<Longrightarrow> \<exists>t' z'. (subst m c,s') \<Rightarrow>\<^sub>G\<^bsup>z'\<^esup> t'"
...
next
  ...
  then show ?case unfolding eq_on_def
    ...
    by (metis (full_types) big_step_tG.Assign)
next
  case (4 c1 s1 C1 s2 c2 C2 s3 C3)
  then show ?case sorry
next
  case (5 s b c1 C t C' c2)
  then show ?case apply auto sorry
next
  case (6 s b c2 C t C' c1)
  then show ?case sorry
next
  case (7 s b c)
  then show ?case sorry
next
  case (8 s1 b c C1 s2 C2 s3 C3)
  then show ?case sorry
qed
