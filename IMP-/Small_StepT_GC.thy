...
theory Small_StepT_GC  imports Big_StepT_Generalized_Cost Main Com Rel_Pow begin
...
locale SS_Generalized_Cost =
  fixes skip_costs :: "'a \<Rightarrow> nat" and 
      assign_costs :: "'a \<Rightarrow> nat"
    assumes assign_cost_positive: "\<forall>x. assign_costs x > 0"
begin

(* In the while case, calculates only current loop call *)

fun cost_of_com :: "com \<Rightarrow> state \<Rightarrow> ('a \<Rightarrow> nat)" where
 "cost_of_com SKIP s = skip_costs" |
 "cost_of_com (Assign x n) s = assign_costs" |
 "cost_of_com (c\<^sub>1;;c\<^sub>2) s = (Plus_lifted (cost_of_com c\<^sub>1 s) (cost_of_com c\<^sub>2 s))" |
 "cost_of_com (IF b \<noteq>0 THEN c\<^sub>1 ELSE c\<^sub>2) s = 
   (if (s b = 0) then cost_of_com c\<^sub>1 s else cost_of_com c\<^sub>2 s)" |
 "cost_of_com (WHILE b\<noteq>0 DO c) s = (if (s b = 0) then (Suc_lifted (cost_of_com c s)) else skip_costs)"


inductive
  small_step :: "com * state \<Rightarrow> ('a \<Rightarrow> nat) \<Rightarrow> com * state \<Rightarrow> bool" ("_ \<rightarrow>\<^sub>G\<^sub>C\<^bsup> _ \<^esup> _" 55) 
where
Assign:  "(x ::= a, s) \<rightarrow>\<^sub>G\<^sub>C\<^bsup> cost_of_com (x ::= a) s \<^esup> (SKIP, s(x := aval a s))" |

Seq1:    "(SKIP;;c\<^sub>2,s) \<rightarrow>\<^sub>G\<^sub>C\<^bsup> cost_of_com (SKIP;;c\<^sub>2) s \<^esup> (c\<^sub>2,s)" |
Seq2:    "(c\<^sub>1,s) \<rightarrow>\<^sub>G\<^sub>C\<^bsup> C1 \<^esup> (c\<^sub>1',s') \<Longrightarrow> (c\<^sub>1;;c\<^sub>2,s) \<rightarrow>\<^sub>G\<^sub>C\<^bsup> cost_of_com (c\<^sub>1;;c\<^sub>2) s \<^esup> (c\<^sub>1';;c\<^sub>2,s')" |

IfTrue:  "s b \<noteq> 0 \<Longrightarrow> (IF b \<noteq>0 THEN c\<^sub>1 ELSE c\<^sub>2,s) \<rightarrow>\<^sub>G\<^sub>C\<^bsup> C1 \<^esup> (c\<^sub>1,s)" |
IfFalse: "s b = 0 \<Longrightarrow> (IF b \<noteq>0  THEN c\<^sub>1 ELSE c\<^sub>2,s) \<rightarrow>\<^sub>G\<^sub>C\<^bsup> C1 \<^esup> (c\<^sub>2,s)" |
