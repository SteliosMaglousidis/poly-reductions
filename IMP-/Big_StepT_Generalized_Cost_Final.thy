section "Big step semantics of IMP- generalized to include 
         ...
         loaded."
theory Big_StepT_Generalized_Cost_Final imports Max_Constant Main Nat_Dom_Lifted Com "HOL-Eisbach.Eisbach_Tools" begin
...
proof
  ...
  then obtain t' s'' where "(c, s) \<Rightarrow>\<^sub>G\<^bsup> t' \<^esup> s''" 
    ...
    by (smt (verit) Assign_tE Seq_costs_split)
...
lemma bigstep_progress:  "(c, s) \<Rightarrow>\<^sub>G\<^bsup> p \<^esup> t \<Longrightarrow> p > 0"
  apply(induct rule: big_step_tG.induct, auto)
    apply (simp add: skip_cost_pos)
  apply (simp add: assign_cost_positive)
  using skip_cost_pos by presburger
...
lemma SKIPp: "\<down>\<^sub>t(SKIP,s) = skip_costs s"
...
lemma costs_exist: "\<exists>z. (c, s) \<Rightarrow>\<^sub>G\<^bsup> z \<^esup> t"
