...
theory Big_StepT_Generalized_Cost imports "../Max_Constant" Main  "../Com" "HOL-Eisbach.Eisbach_Tools" begin
...
locale BS_Generalized_Cost =
  fixes skip_costs :: "'a \<Rightarrow> nat" and 
      assign_costs :: "'a \<Rightarrow> nat"
    assumes assign_cost_positive: "\<forall>x. assign_costs x > 0"
begin 

inductive
  big_step_tG :: "com \<times> state \<Rightarrow> ('a \<Rightarrow> nat) \<Rightarrow> state \<Rightarrow> bool"  ("_ \<Rightarrow>\<^bsup> _ \<^esup> _" 55)
where
Skip: "(SKIP,s) \<Rightarrow>\<^bsup> skip_costs \<^esup> s"|
Assign: "(x ::= a,s) \<Rightarrow>\<^bsup> assign_costs \<^esup> s(x := aval a s)" |
Seq: "\<lbrakk> (c1,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s2;  (c2,s2) \<Rightarrow>\<^bsup> C2 \<^esup> s3 ; C3 = (\<lambda>x. (C1 x) + (C2 x)) \<rbrakk> \<Longrightarrow> (c1;;c2, s1) \<Rightarrow>\<^bsup> C3 \<^esup> s3" |
IfTrue: "\<lbrakk> s b \<noteq> 0;  (c1,s) \<Rightarrow>\<^bsup> C \<^esup> t; C'= (\<lambda>x. (C x) + 1) \<rbrakk> \<Longrightarrow> (IF b \<noteq>0 THEN c1 ELSE c2, s) \<Rightarrow>\<^bsup> C' \<^esup> t" |
IfFalse: "\<lbrakk> s b = 0; (c2,s) \<Rightarrow>\<^bsup> C \<^esup> t; C'= (\<lambda>x. (C x) + 1)  \<rbrakk> \<Longrightarrow> (IF b \<noteq>0 THEN c1 ELSE c2, s) \<Rightarrow>\<^bsup> C' \<^esup> t" |
WhileFalse: "\<lbrakk> s b = 0 \<rbrakk> \<Longrightarrow> (WHILE b \<noteq>0 DO c,s) \<Rightarrow>\<^bsup> (\<lambda>x. 2 * (skip_costs x)) \<^esup> s" |
WhileTrue: "\<lbrakk> s1 b \<noteq> 0;  (c,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s2;  (WHILE b \<noteq>0 DO c, s2) \<Rightarrow>\<^bsup> C2 \<^esup> s3; C3 = (\<lambda>x. (C1 x) + (C2 x) + 1)\<rbrakk> 
    \<Longrightarrow> (WHILE b \<noteq>0 DO c, s1) \<Rightarrow>\<^bsup> C3 \<^esup> s3" 
end

bundle big_stepG_syntax
begin
notation BS_Generalized_Cost.big_step_tG ("_ \<Rightarrow>\<^bsup> _ \<^esup> _" 55)
end

bundle no_big_stepG_syntax 
begin
no_notation BS_Generalized_Cost.big_step_tG ("_ \<Rightarrow>\<^bsup> _ \<^esup> _" 55)
end
...
text "A more general, elegant version of those would be nice"
...
lemmas big_step_tG_induct =  BS_Generalized_Cost.big_step_tG.induct[split_format(complete)]

subsection "Rule inversion"
inductive_cases Skip_tE[elim!]: "(SKIP,s) \<Rightarrow>\<^bsup> C \<^esup> t"
inductive_cases Assign_tE[elim!]: "(x ::= a,s) \<Rightarrow>\<^bsup> C \<^esup> t"
inductive_cases Seq_tE[elim!]: "(c1;;c2,s1) \<Rightarrow>\<^bsup> C \<^esup> s2"
inductive_cases If_tE[elim!]: "(IF b \<noteq>0 THEN c1 ELSE c2,s) \<Rightarrow>\<^bsup> C \<^esup> t"
inductive_cases While_tE[elim]: "(WHILE b \<noteq>0 DO c,s) \<Rightarrow>\<^bsup> C \<^esup> t"
lemma Seq': "\<lbrakk>(c1,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s2;  (c2,s2) \<Rightarrow>\<^bsup> C2 \<^esup> s3\<rbrakk> \<Longrightarrow> (c1;;c2, s1) \<Rightarrow>\<^bsup> (\<lambda>x. C1 x + C2 x) \<^esup> s3"
  by (simp add: Seq)
lemma While_tE': "\<lbrakk> s1 b \<noteq> 0;  (c,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s2;  (WHILE b \<noteq>0 DO c, s2) \<Rightarrow>\<^bsup> C2 \<^esup> s3\<rbrakk> 
    \<Longrightarrow> (WHILE b \<noteq>0 DO c, s1) \<Rightarrow>\<^bsup> (\<lambda>x. (C1 x) + (C2 x) + 1) \<^esup> s3"
  by (simp add: big_step_tG.WhileTrue)

thm Seq_tE


lemma Seq_costs_split: "(c1;;c2, s1) \<Rightarrow>\<^bsup> C \<^esup> s3 \<Longrightarrow> \<exists>C1 C2 s2. ((c1,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s2) \<and>  ((c2,s2) \<Rightarrow>\<^bsup> C2 \<^esup> s3) \<and> C = (\<lambda>x. (C1 x) + (C2 x))" 
  apply (erule Seq_tE)
  by auto

lemma While_costs_split: "\<lbrakk> s1 b \<noteq> 0;  (WHILE b \<noteq>0 DO c, s1) \<Rightarrow>\<^bsup> C \<^esup> s3\<rbrakk> 
    \<Longrightarrow> \<exists>C1 C2 s2. ((c,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s2) \<and> ((WHILE b \<noteq>0 DO c, s2) \<Rightarrow>\<^bsup> C2 \<^esup> s3) \<and> C = (\<lambda>x. (C1 x) + (C2 x) + 1)"
  by force
...
lemma "(IF b \<noteq>0 THEN SKIP ELSE SKIP, s) \<Rightarrow>\<^bsup> C \<^esup> t \<Longrightarrow> t = s"
  by blast

lemma assumes "(IF b \<noteq>0 THEN SKIP ELSE SKIP, s) \<Rightarrow>\<^bsup> C \<^esup> t"
  shows "t = s"
  using assms apply cases by auto

lemma assign_t_simp:
  "((x ::= a,s) \<Rightarrow>\<^bsup> assign_costs \<^esup>  s') \<longleftrightarrow> (s' = s(x := aval a s))"
  using big_step_tG.Assign by auto

lemma seq_costs_split: "(c1;;c2,s1) \<Rightarrow>\<^bsup> C \<^esup> s2 \<Longrightarrow> 
                        \<exists>C1 C2 s. ((c1,s1) \<Rightarrow>\<^bsup> C1 \<^esup> s) \<and> ((c2,s) \<Rightarrow>\<^bsup> C2 \<^esup> s2)
                        \<and> C = (\<lambda>x. (C1 x) + (C2 x)) " by auto

subsection "Determinism of Big semantics of IMP-"
theorem big_step_t_state_determ2: "\<lbrakk> (c,s) \<Rightarrow>\<^bsup> p \<^esup> t; (c,s) \<Rightarrow>\<^bsup> q \<^esup> u \<rbrakk> \<Longrightarrow> (u = t)"
...
theorem big_step_t_cost_determ2: "\<lbrakk> (c,s) \<Rightarrow>\<^bsup> p \<^esup> t; (c,s) \<Rightarrow>\<^bsup> q \<^esup> t \<rbrakk> \<Longrightarrow> p = q"
...
next
  ...
  then show ?case  proof-
   ...
   ((c, s1) \<Rightarrow>\<^bsup> q1 \<^esup> s2') \<and>
   ((WHILE b\<noteq>0 DO c, s2') \<Rightarrow>\<^bsup> q2 \<^esup> s3) \<and> q = (\<lambda>x. q1 x + q2 x + 1)"
      using WhileTrue.hyps(1) WhileTrue.prems by auto
    have "s2' = s2" 
      using WhileTrue.hyps(2) \<open>(c, s1) \<Rightarrow>\<^bsup> q1 \<^esup> s2' \<and> (WHILE b\<noteq>0 DO c, s2') \<Rightarrow>\<^bsup> q2 \<^esup> s3 \<and> q = (\<lambda>x. q1 x + q2 x + 1)\<close> big_step_t_state_determ2 by auto
    moreover have "q1 = C1"
    proof- have "(c, s1) \<Rightarrow>\<^bsup> q1 \<^esup> s2" 
        using \<open>(c, s1) \<Rightarrow>\<^bsup> q1 \<^esup> s2' \<and> (WHILE b\<noteq>0 DO c, s2') \<Rightarrow>\<^bsup> q2 \<^esup> s3 \<and> q = (\<lambda>x. q1 x + q2 x + 1)\<close> calculation by auto
      thus ?thesis 
        using WhileTrue.IH(1) by auto
    qed
    moreover have "q2 = C2"
    proof- have "(WHILE b\<noteq>0 DO c, s2) \<Rightarrow>\<^bsup> q2 \<^esup> s3" 
        using \<open>((c, s1) \<Rightarrow>\<^bsup> q1 \<^esup> s2') \<and>
   ((WHILE b\<noteq>0 DO c, s2') \<Rightarrow>\<^bsup> q2 \<^esup> s3) \<and> q = (\<lambda>x. q1 x + q2 x + 1)\<close> calculation by auto
      thus ?thesis 
        by (simp add: WhileTrue.IH(2))
    qed
    moreover have "q = (\<lambda>x. C1 x + C2 x + 1)" 
    proof-
      have " q = (\<lambda>x. q1 x + q2 x + 1)" using \<open>(c, s1) \<Rightarrow>\<^bsup> q1 \<^esup> s2' \<and> (WHILE b\<noteq>0 DO c, s2') \<Rightarrow>\<^bsup> q2 \<^esup> s3 \<and> q = (\<lambda>x. q1 x + q2 x + 1)\<close>
        by simp
...
lemma seq_assign_t_simp:
  "((c ;; x ::= a, s) \<Rightarrow>\<^bsup> (\<lambda>x. t x + assign_costs x) \<^esup>  s') 
  \<longleftrightarrow> (\<exists>s''. ((c, s) \<Rightarrow>\<^bsup> t \<^esup> s'') \<and> s' = s''(x := aval a s''))"
proof
  assume "(c;; x ::= a, s) \<Rightarrow>\<^bsup> (\<lambda>x. t x + assign_costs x) \<^esup> s'"
  thm seq_costs_split
  then obtain C1 C2 s'' where "(c, s) \<Rightarrow>\<^bsup> C1 \<^esup> s''" 
                          and "(x ::= a, s'') \<Rightarrow>\<^bsup> C2 \<^esup> s'" 
                          and " (\<lambda>x. t x + assign_costs x) = (\<lambda>x. C1 x + C2 x)"
    using \<open>(c;; x ::= a, s) \<Rightarrow>\<^bsup> (\<lambda>x. t x + assign_costs x) \<^esup> s'\<close> seq_costs_split 
    by meson
  have "assign_costs = C2" using \<open>(c;; x ::= a, s) \<Rightarrow>\<^bsup>  (\<lambda>x. t x + assign_costs x) \<^esup> s'\<close>
    using \<open>(x ::= a, s'') \<Rightarrow>\<^bsup> C2 \<^esup> s'\<close> by auto
  ...
  thus "\<exists>s''. ((c, s) \<Rightarrow>\<^bsup> t \<^esup> s'') \<and> s' = s''(x := aval a s'')"
    using \<open>(c, s) \<Rightarrow>\<^bsup> C1 \<^esup> s''\<close> \<open>(x ::= a, s'') \<Rightarrow>\<^bsup> C2 \<^esup> s'\<close> by auto
next
  assume "\<exists>s''. ((c, s) \<Rightarrow>\<^bsup> t \<^esup> s'') \<and> s' = s''(x := aval a s'')"
  thus "(c ;; x ::= a, s) \<Rightarrow>\<^bsup> (\<lambda>x. t x + assign_costs x) \<^esup>  s'"
    using Seq' big_step_tG.Assign by auto
qed 

lemma seq_assign_t_intro: "(c, s) \<Rightarrow>\<^bsup> t \<^esup> s'' \<Longrightarrow> s' = s''(x := aval a s'')
  \<Longrightarrow>(c ;; x ::= a, s) \<Rightarrow>\<^bsup> (\<lambda>x. t x + assign_costs x) \<^esup>  s'"
  using seq_assign_t_simp 
  by auto

lemma seq_is_noop[simp]: "((SKIP, s) \<Rightarrow>\<^bsup>t\<^esup> s') \<longleftrightarrow> (t = skip_costs \<and> s = s')" 
  using Skip by auto

lemma seq_skip[simp]: "((c ;; SKIP, s) \<Rightarrow>\<^bsup>(\<lambda>x. t x + skip_costs x)\<^esup> s') \<longleftrightarrow> ((c, s) \<Rightarrow>\<^bsup>t\<^esup> s')"
  apply auto 
...
lemma bigstep_progress:  "(c, s) \<Rightarrow>\<^bsup> p \<^esup> t \<Longrightarrow> \<forall>x. skip_costs x > 0 \<Longrightarrow>  \<forall>x. p x > 0"
...
lemma SKIPnot: "(\<not> ((SKIP, s) \<Rightarrow>\<^bsup> p \<^esup> t)) \<longleftrightarrow> (s\<noteq>t \<or> p\<noteq>skip_costs)" by auto
...
lemma ASSnot: "( \<not>((x ::= e, s) \<Rightarrow>\<^bsup> p \<^esup> t )) = (p\<noteq>assign_costs \<or> t\<noteq>s(x := aval e s))"
  apply auto
...
lemma AssignD': 
"(x ::= a, s) \<Rightarrow>\<^bsup> assign_costs \<^esup> s' \<Longrightarrow> s' = s (x:= aval a s)"
...
lemma Seq'': "\<lbrakk> ((c1,s1) \<Rightarrow>\<^bsup> x \<^esup> s2) \<and> P s2; P s2 \<Longrightarrow> ((c2,s2) \<Rightarrow>\<^bsup> y \<^esup> s3) \<and> Q s3; Q s3 \<Longrightarrow> R s3 \<rbrakk>
             \<Longrightarrow> ((c1;;c2, s1) \<Rightarrow>\<^bsup> (\<lambda>z. (x z) + (y z))\<^esup> s3) \<and> R s3"
...
lemma IfE: 
"(IF b \<noteq>0 THEN c1 ELSE c2, s) \<Rightarrow>\<^bsup> (\<lambda>x. (if s b \<noteq> 0 then x1 else x2) x + 1) \<^esup> (if s b \<noteq> 0 then s1 else s2) \<Longrightarrow> 
...
lemma IfD:
"(IF b \<noteq>0 THEN c1 ELSE c2, s) \<Rightarrow>\<^bsup> (\<lambda>x. (if s b \<noteq> 0 then x1 else x2) x + 1) \<^esup> (if s b \<noteq> 0 then t1 else t2) \<Longrightarrow> 
...
"\<lbrakk>s' = s (x:= aval a s)\<rbrakk>
        \<Longrightarrow> (x ::= a, s) \<Rightarrow>\<^bsup> assign_costs \<^esup> s'" 
...
lemma AssignI'': 
"\<lbrakk>s' = s (x:= aval a s)\<rbrakk>
        \<Longrightarrow> ((x ::= a, s) \<Rightarrow>\<^bsup> assign_costs \<^esup> s') \<and> (s' = s')" 
  by (auto simp add: Assign eval_nat_numeral)

lemma AssignD: "(x ::= a, s) \<Rightarrow>\<^bsup> t \<^esup> s' \<Longrightarrow> t = assign_costs \<and> s' = s(x := aval a s)"
  by auto

lemma compose_programs_1:
  "(c2, s2) \<Rightarrow>\<^bsup> y \<^esup> s3 \<Longrightarrow> (c1, s1) \<Rightarrow>\<^bsup> x \<^esup> s2 \<Longrightarrow> 
    ((c1;; c2, s1) \<Rightarrow>\<^bsup> (\<lambda>z. x z + y z) \<^esup> s3 \<Longrightarrow> P)
   \<Longrightarrow> P" 
  by (simp add: Seq')
...
"(WHILE b\<noteq>0 DO c, s) \<Rightarrow>\<^bsup> x \<^esup> t \<Longrightarrow>
  ...
  (\<And>x' s2 y. 0 < s b \<Longrightarrow> (c, s) \<Rightarrow>\<^bsup> x' \<^esup> s2 \<Longrightarrow> (WHILE b\<noteq>0 DO c, s2) \<Rightarrow>\<^bsup> y \<^esup> t \<Longrightarrow> x = (\<lambda>z. x' z + y z + 1) \<Longrightarrow> P) \<Longrightarrow> P"
  by auto
...
lemma Seq_tE_While_init:
  "(WHILE v \<noteq>0 DO c2, s2) \<Rightarrow>\<^bsup> y \<^esup> s3 \<Longrightarrow> (c1, s1) \<Rightarrow>\<^bsup> x \<^esup> s2 \<Longrightarrow> 
    ((c1;; WHILE v \<noteq>0 DO c2, s1) \<Rightarrow>\<^bsup> (\<lambda>z. x z + y z) \<^esup> s3 \<Longrightarrow> P)
   \<Longrightarrow> P"
