...
lemmas big_step_t'_GC_induct  = BS_Generalized_Cost.big_step_t'_GC.induct[split_rule, OF BS_Generalized_Cost_axioms]
...
lemma noninterference: 
  ...
  sorry (* proof (induction c s x t arbitrary: s' rule: big_step_t'_induct)
  case (Assign' x a s)
  then show ?case 
...
lemma ssubst_unchanged: "big_step_tG ((ssubst m vs),s) z t \<Longrightarrow> (\<forall>v\<in>set vs. m v \<notin> set vs) \<Longrightarrow> s = t on set vs"
  unfolding eq_on_def using lvars_ssubst lvars_unchanged image_iff try (* by (metis (mono_tags, lifting)) *)
...
next
  case (Cons a vs)
  then show ?case proof-
    have "((m a) ::= A (V a) ;; ssubst m vs, s) \<Rightarrow>\<^sub>G\<^bsup> z \<^esup> t" 
      using Cons.prems ssubst.simps(2) by auto
    obtain t' where 
qed
...
next
  ...
  from Cons obtain t where
    2: "(ssubst m vs, s(m v := s v)) \<Rightarrow>\<^sub>G\<^bsup> ssubst_costs m s vs \<^esup> t" and
    s: "s(m v := s v) = t \<circ> m on set vs" sorry
  ...
  next
    ...
    hence "(s(m v := s v)) (m v) = t (m v)"  sorry (* using var_unchanged[OF 2] by blast *)
  ...
  with 1 2 show ?case unfolding eq_on_def
    apply (simp add: numeral_eq_Suc) sorry (*
...
lemma transfer_sound:
  assumes c_sem: "(c,s) \<Rightarrow>\<^bsup>z\<^esup> t"
    ...
    obtains t'
    where "(transfer m c,s')\<Rightarrow>\<^bsup>skip_costs t' + assign_costs_of_list m s (vars c) + z\<^esup> t'"
      and "t = t' o m on set (vars c)"
proof -
  from ssubst_correct[OF inj disj] obtain s\<^sub>2 where ssubst_sem:
    "(ssubst m (vars c),s')\<Rightarrow>\<^bsup>skip_costs s\<^sub>2 + assign_costs_of_list m s' (vars c)\<^esup> s\<^sub>2" and
  ...
  with ssubst_sem have "(transfer m c,s')\<Rightarrow>\<^bsup>skip_costs t' + assign_costs_of_list m s' (vars c) + z\<^esup> t'"
    unfolding transfer_def sorry
  with t' that show ?thesis sorry
qed
...
lemma transfer_complete:
  assumes transfer_sem: "(transfer m c,s')\<Rightarrow>\<^sub>G\<^bsup>z'\<^esup> t'"
      ...
      and disj: "(\<forall>v\<in>set (vars c). m v \<notin> set (vars c))"
  obtains t z
    where "(c,s) \<Rightarrow>\<^sub>G\<^bsup>z\<^esup> t" "z' = Suc_lifted (Plus_lifted (times_lifted assign_cost  (\<lambda>x. length (vars c))) z)" "t = t' o m on set (vars c)"
proof -
  ...
  with transfer_sem[unfolded transfer_def] obtain z where
   subst_sem: "(subst m c,s\<^sub>2') \<Rightarrow>\<^sub>G\<^bsup>z\<^esup> t'" and z: "z' = Suc_lifted (Plus_lifted (times_lifted assign_cost  (\<lambda>x. length (vars c))) z)"
    using bigstep_det  sorry (* by fastforce *)
  from s s\<^sub>2' have "s = s\<^sub>2' o m on set (vars c)" by fastforce 
(*
  from subst_complete[OF subst_sem this _ inj] that z *) show ?thesis sorry
qed
...
proof -
  ...
  from ssubst_correct[OF inj disj] have "s = s\<^sub>2 o m on set (vars c)"
    using \<open>(ssubst m (vars c), s) \<Rightarrow>\<^sub>G\<^bsup> x \<^esup> s\<^sub>2\<close> bigstep_det sorry

  have "s = s\<^sub>2 on UNIV - m ` set (vars c)"
    using disj ssubst_sem ssubst_unchanged  sorry
