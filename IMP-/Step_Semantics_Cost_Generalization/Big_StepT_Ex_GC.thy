...
theory Big_StepT_Ex_GC
  imports Big_StepT_Generalized_Cost "~~/src/HOL/Library/Simps_Case_Conv"
begin
                  
context BS_Generalized_Cost begin      

function (sequential, domintros) big_step_function :: "(com \<times> state) \<Rightarrow> (('a \<Rightarrow> nat) \<times> state)" where
  "big_step_function (SKIP,s) = (skip_costs, s)" |
  "big_step_function (x ::= a,s) = (assign_costs, s(x := aval a s))" |
  "big_step_function (c1;;c2, s) =
    (let (n1, s1) = big_step_function (c1, s);
        (n2, s2) = big_step_function (c2, s1)
    in (Plus_lifted n1 n2, s2))" |
  "big_step_function (IF b \<noteq>0 THEN c1 ELSE c2, s) = 
    apfst Suc_lifted (if s b \<noteq> 0 then big_step_function (c1,s) else big_step_function (c2,s))" |
  "big_step_function (WHILE b \<noteq>0 DO c,s) = (if s b \<noteq> 0 then apfst Suc_lifted (big_step_function (c;; WHILE b \<noteq>0 DO c, s)) else (while_exit_costs, s))" 
...
next
  ...
  then show ?case 
    ...
    by (simp add: Plus_lifted_def) (* ? ? ? *)
next
  case (IfTrue s b c1 C t C' c2)
  then show ?case apply (auto intro: big_step_function.domintros simp add: Let_def big_step_function.domintros big_step_function.psimps)
    by (simp add: Suc_lifted_def)
next
...
next
  ...
  show ?case 
    ...
    using WhileTrue.IH(1) WhileTrue.IH(2) WhileTrue.hyps(4) 
      ...
      apply (simp add: Plus_lifted_def Suc_lifted_def)
     apply (simp add: WhileTrue.hyps(1))
...
lemma mono1[partial_function_mono]: "monotone option.le_fun option_ord
          (\<lambda>big_step_ex. map_option (apfst ((Plus_lifted) aa)) (big_step_ex (x32, ba)))" 
...
lemma mono2[partial_function_mono]:"
         monotone option.le_fun option_ord
          (\<lambda>f. map_option (apfst Suc_lifted) (f (x52;; WHILE x51\<noteq>0 DO x52, b)))"
  by (smt (verit, best) flat_ord_def fun_ord_def monotoneI option.map_disc_iff)
lemma mono3[partial_function_mono]:"
  ...
  (\<And>x a b x51 x52.
      monotone option.le_fun option_ord
       (\<lambda>f. map_option (apfst Suc_lifted) (f (x52;; WHILE x51\<noteq>0 DO x52, b)))) \<Longrightarrow>
  (\<And>x. monotone option.le_fun option_ord
         (\<lambda>big_step_ex.
             case x of (SKIP, s) \<Rightarrow> Some (skip_costs, s) | (x ::= a, s) \<Rightarrow> Some (assign_costs, s(x := aval a s))
             | (c1;; c2, s) \<Rightarrow>
                 Option.bind (big_step_ex (c1, s))
                  (\<lambda>(n, s). map_option (apfst ((Plus_lifted) n)) (big_step_ex (c2, s)))
             ...
             | (WHILE b\<noteq>0 DO c, s) \<Rightarrow>
                 if s b \<noteq> 0 then map_option (apfst Suc_lifted) (big_step_ex (c;; WHILE b\<noteq>0 DO c, s))
                 else Some (while_exit_costs, s)))"
...
partial_function (option) big_step_ex :: "(com\<times> state) \<Rightarrow> (('a \<Rightarrow> nat) \<times> state) option" where
  "big_step_ex cs = (case cs of (SKIP,s) \<Rightarrow> Some (skip_costs, s) | (x ::= a,s) \<Rightarrow> Some (assign_costs, s(x := aval a s))
    | (c1;;c2, s1) \<Rightarrow> Option.bind (big_step_ex (c1,s1)) (\<lambda>(n, s). map_option (apfst (\<lambda>n'. Plus_lifted n n')) (big_step_ex (c2, s)))
    | (IF b \<noteq>0 THEN c1 ELSE c2, s) \<Rightarrow> map_option (apfst Suc_lifted) (if s b \<noteq> 0 then big_step_ex (c1,s) else big_step_ex (c2,s))
    | (WHILE b \<noteq>0 DO c,s) \<Rightarrow> (if s b \<noteq> 0 then map_option (apfst Suc_lifted) (big_step_ex (c;; WHILE b \<noteq>0 DO c, s)) else Some (while_exit_costs, s)))"
...
lemma big_step_ex_sound:"(c,s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow> big_step_ex (c,s) = Some (t, s')"
  by (induction rule: big_step_tG.induct) (auto simp add: Suc_lifted_def Plus_lifted_def)
