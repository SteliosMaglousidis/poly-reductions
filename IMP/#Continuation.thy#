theory Continuation
  imports IMP_GRec Last_Rec_Call "HOL-Library.Tree"
begin
...
lemma last_call_continuate: "(continuate d r) \<turnstile>\<^sub>L\<^sub>C ((continuate c r),s) \<Rightarrow>\<^bsup>z\<^esup> (t,r') \<Longrightarrow> the r' = r \<or> r' = None"
...
lemma last_call_continuate_var: "(continuate d r) \<turnstile>\<^sub>L\<^sub>C ((continuate c r),s) \<Rightarrow>\<^bsup>z\<^esup> (t, Some r') \<Longrightarrow> r' = r"
  by (metis last_call_continuate option.distinct(1) option.sel)

lemma last_call_continuate_val_var: 
  "(continuate d r) \<turnstile>\<^sub>L\<^sub>C\<^sub>V ((continuate c r),s) \<Rightarrow>\<^bsup>z\<^esup> (t, Some v) \<Longrightarrow> 
   (continuate d r) \<turnstile>\<^sub>L\<^sub>C ((continuate c r),s) \<Rightarrow>\<^bsup>z\<^esup> (t, Some r)"
  by (metis last_call_continuate_var last_call_value1)

lemma last_call_value: 
"\<lbrakk>cr' \<turnstile>\<^sub>L\<^sub>C\<^sub>V (cr,s) \<Rightarrow>\<^bsup> z \<^esup> (t, v); v = Some val; cr' = continuate c' r; cr = continuate c r;
  set (vars c) \<subseteq> S; set (vars c') \<subseteq> S; r \<notin> S\<rbrakk>
   \<Longrightarrow> t r = val"
proof(induction cr' cr s z t v arbitrary: c' c r val rule: rlast_call_val_induct)
  case (rlcvSkip c s)
...
next
  case (rlcvSeq1 cr cr1 s1 x s2 v cr2 y s3 z)
