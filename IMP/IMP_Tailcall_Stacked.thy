theory IMP_Tailcall_Stacked
  imports IMP_Calls IMP_GRec
begin

unbundle no com_syntax and com'_syntax
declare [[syntax_ambiguity_warning=false]]

(* Intuition is basically the values to be consumed by each call *)
type_synonym stack = "val list" 

fun pop :: "stack \<Rightarrow> stack" where
"pop [] = []" |
"pop (x#xs) =  xs" 

fun push :: "stack \<Rightarrow> val \<Rightarrow> stack" where
"push xs v = v#xs"

type_synonym fstack = "stack list"

fun remove_i :: "nat \<Rightarrow> 'a list \<Rightarrow> 'a list" where
"remove_i i [] = []" |
"remove_i 0 (x#xs) = xs" |
"remove_i (Suc i) (x#xs) = x#(remove_i i xs)"

abbreviation "push_i i v fs \<equiv> (if length fs > i then list_update fs i (v # fs ! i) else [v] # fs)"
abbreviation "pop_i i fs \<equiv> (if length (fs ! i) = 1  then remove_i i fs 
                              else (if length fs > i then list_update fs i (tl (fs ! i)) 
                                else (pop (hd fs)) # tl fs))"
...
where
tsSkip: "c \<turnstile> (tsSKIP,s,ss) \<Rightarrow>\<^bsup>Suc (0::nat) \<^esup> (s,ss)" |
tsPush: "c \<turnstile>(Push v i,s,ss) \<Rightarrow>\<^bsup>Suc (Suc 0) \<^esup> (s, push_i i (s v) ss)" |
tsPop: "c \<turnstile>(Pop i,s,ss) \<Rightarrow>\<^bsup>Suc (Suc 0) \<^esup> (s, pop_i i ss)" |
tAssign: "c \<turnstile>(x ::= a,s,ss) \<Rightarrow>\<^bsup>Suc (Suc 0) \<^esup> (s(x := aval a s),ss)" |
tSeq: "\<lbrakk>c \<turnstile> (c1,s1,ss1) \<Rightarrow>\<^bsup>x \<^esup> (s2,ss2) ; c \<turnstile> (c2,s2,ss2) \<Rightarrow>\<^bsup>y \<^esup> (s3,ss3) ; z=x+y\<rbrakk> \<Longrightarrow> c \<turnstile> (c1;;c2, s1, ss1) \<Rightarrow>\<^bsup>z \<^esup> (s3,ss3)" |
tIfTrue: "\<lbrakk>s b \<noteq> 0;  c \<turnstile> (c1,s,ss) \<Rightarrow>\<^bsup>x \<^esup> (t,st); y=x+1\<rbrakk> \<Longrightarrow> c \<turnstile> (IF b \<noteq>0 THEN c1 ELSE c2,s,ss) \<Rightarrow>\<^bsup>y \<^esup> (t,st)" |
tIfFalse: "\<lbrakk>s b = 0; c \<turnstile> (c2,s,ss) \<Rightarrow>\<^bsup>x \<^esup> (t,st); y=x+1\<rbrakk> \<Longrightarrow> c \<turnstile> (IF b \<noteq>0 THEN c1 ELSE c2,s,ss) \<Rightarrow>\<^bsup>y \<^esup> (t,st)" |
tCall: "(C,s) \<Rightarrow>\<^bsup> z \<^esup> t \<Longrightarrow> c \<turnstile> (TCALL C RETURN r,s,ss) \<Rightarrow>\<^bsup>z \<^esup> (s(r:=t r),ss)" |
\<comment> \<open>New rule\<close>
tTail: "c \<turnstile> (c,s,ss) \<Rightarrow>\<^bsup>z \<^esup> (t,ts) \<Longrightarrow> c \<turnstile> (tsTAIL,s,ss) \<Rightarrow>\<^bsup>5 + z \<^esup> (t,ts)"
...
inductive_cases tCall_tE[elim!]: "c \<turnstile> (TCALL C RETURN v,s) \<Rightarrow>\<^bsup>z \<^esup> t"
...
"vars_tscom (TCALL c RETURN r) = r#vars c" |
"vars_tscom _ = []"
...
fun iflag :: "nat \<Rightarrow> vname" where
"iflag n = ''Dedicated name for stack flag n''"

fun tree_to_stails :: "rcom \<Rightarrow> nat \<Rightarrow> tscom" where
"tree_to_stails rSKIP n = tsSKIP" |
"tree_to_stails (x ::= a) n = (x ::= a)"|
(* *)
"tree_to_stails (CALL C RETURN r;; c2) n = (IF iflag n\<noteq>0 THEN (Pop n ;; (tree_to_stails c2 (n + 1)) ;; Push r (n + 1)) ELSE tsSKIP)"|
"tree_to_stails (c1;;c2) n = (tree_to_stails c1 n ;; tree_to_stails c2 n)"|
...
value "tree_to_stails (tree_cf ( 
     (IF n\<noteq>0 THEN 
             ...
             ELSE (r ::= A (N 1))))) 0"
(*
theorem "d \<turnstile>\<^sub>R (c,s) \<Rightarrow>\<^bsup>z \<^esup> t 
  \<Longrightarrow> (tree_to_stails (tree_cf d) 0) \<turnstile> (tree_to_stails (tree_cf c) 0,s,[]) \<Rightarrow>\<^bsup>z \<^esup> (t,[])"
*)
...
inductive
  stail_step :: "(tscom \<times> state) \<Rightarrow> nat \<Rightarrow> (state \<times> bool) \<Rightarrow> bool" ("\<turnstile>_ \<Rightarrow>\<^bsup>_\<^esup>  _" 55)
where
Skip: "\<turnstile> (tsSKIP,s) \<Rightarrow>\<^bsup>Suc (0::nat) \<^esup> (s,False)" |
...
Call: "(C,s) \<Rightarrow>\<^bsup>z \<^esup> t \<Longrightarrow> \<turnstile> (TCALL C RETURN r,s) \<Rightarrow>\<^bsup>z \<^esup> (s(r:=t r),False)" |
Tail: "\<turnstile> (tsTAIL,s) \<Rightarrow>\<^bsup>5 \<^esup> (s,True)"
...
inductive_cases stailCall_tE[elim!]: "\<turnstile> (TCALL C RETURN v,s) \<Rightarrow>\<^bsup>z \<^esup> (t,b)"

inductive_cases stailTail_tE[elim]: "\<turnstile> (tsTAIL,s) \<Rightarrow>\<^bsup>x \<^esup> (t,b)"

text \<open>Closure of tails\<close>
inductive
  stail_steps :: "tscom \<Rightarrow> tscom \<times> state \<Rightarrow> nat \<Rightarrow> state \<Rightarrow> bool" ("_ \<turnstile>''_ \<Rightarrow>\<^bsup>_\<^esup>  _" 55) for d
where
...
lemma no_tails_no_step: "\<turnstile>(c,s) \<Rightarrow>\<^bsup>z \<^esup> (t,b) \<Longrightarrow> \<not>stails c \<Longrightarrow> \<not>b"
  by (induction c s z t b rule: stail_step_induct) auto

lemma no_tails_sem: "\<turnstile>(c,s) \<Rightarrow>\<^bsup>z \<^esup> (t,b) \<Longrightarrow> \<not>stails c \<Longrightarrow> \<exists>ts .  d \<turnstile> (c,s,ss) \<Rightarrow>\<^bsup>z \<^esup> (t,ts)"
  by (induction c s z t b arbitrary: ss rule: stail_step_induct) fastforce+

lemma small_sound: "d \<turnstile> (c,s,ss) \<Rightarrow>\<^bsup>z \<^esup> (t,ts) \<Longrightarrow> sinvar c \<Longrightarrow> sinvar d \<Longrightarrow> d \<turnstile>' (c,s) \<Rightarrow>\<^bsup>z \<^esup> t"
proof (induction d c s ss z t ts rule: tsbig_step_t_induct)
