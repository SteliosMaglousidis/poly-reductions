...
inductive
  upto_trec :: "(tscom \<times> state \<times> fstack) \<Rightarrow> nat \<Rightarrow> (state \<times> fstack \<times> bool) \<Rightarrow> bool" ("\<turnstile>UPTO\<lparr> _ \<Rightarrow>\<^bsup>_\<^esup>  _" 55)
where              
Skip: "\<turnstile>UPTO\<lparr> (SKIP,s,stack) \<Rightarrow>\<^bsup>Suc (0::nat) \<^esup> (s,stack,False)" |
Assign: "\<turnstile>UPTO\<lparr> (x ::= a,s,stack) \<Rightarrow>\<^bsup>Suc (Suc 0) \<^esup> (s(x := aval a s),stack,False)" |
Seq1: "\<lbrakk>\<turnstile>UPTO\<lparr> (c1,s1,stack1) \<Rightarrow>\<^bsup>x \<^esup> (s2,stack2,True)\<rbrakk> \<Longrightarrow> \<turnstile>UPTO\<lparr> (c1;;c2,s1,stack1) \<Rightarrow>\<^bsup>x\<^esup> (s2,stack2,True)" |
Seq2: "\<lbrakk>\<turnstile>UPTO\<lparr> (c1,s1,stack1) \<Rightarrow>\<^bsup>x \<^esup> (s2,stack2,False); \<turnstile>UPTO\<lparr> (c2,s2,stack2) \<Rightarrow>\<^bsup>y \<^esup> (s3,stack3,CONT); z=x+y\<rbrakk> \<Longrightarrow> \<turnstile>UPTO\<lparr> (c1;;c2,s1,stack1) \<Rightarrow>\<^bsup>z\<^esup> (s3,stack3,CONT)" |
IfTrue: "\<lbrakk> s b \<noteq> 0; \<turnstile>UPTO\<lparr> (c1,s,stack) \<Rightarrow>\<^bsup>x \<^esup> (s',stack',CONT); y=x+1 \<rbrakk> \<Longrightarrow> \<turnstile>UPTO\<lparr> (IF b \<noteq>0 THEN c1 ELSE c2,s,stack) \<Rightarrow>\<^bsup>y \<^esup> (s',stack',CONT)" |
IfFalse: "\<lbrakk> s b = 0; \<turnstile>UPTO\<lparr> (c2,s,stack) \<Rightarrow>\<^bsup>x \<^esup> (s',stack',CONT); y=x+1  \<rbrakk> \<Longrightarrow> \<turnstile>UPTO\<lparr> (IF b \<noteq>0 THEN c1 ELSE c2,s,stack) \<Rightarrow>\<^bsup>y \<^esup> (s',stack',CONT)" |
Call: "(C,s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow> \<turnstile>UPTO\<lparr> (CALL C RETURN r,s,stack) \<Rightarrow>\<^bsup>t \<^esup> (s(r:=s' r),stack,False)" |
Rec: "\<turnstile>UPTO\<lparr> (TAIL,s,stack) \<Rightarrow>\<^bsup>5 \<^esup> (s,stack,True)"

code_pred upto_trec .

declare upto_trec.intros[intro]

lemmas upto_trec_induct = upto_trec.induct[split_format(complete)]

inductive_cases uptoSkip_tE[elim!]: "\<turnstile>UPTO\<lparr> (SKIP,s,stack) \<Rightarrow>\<^bsup>x \<^esup> (t,b)"
inductive_cases uptoAssign_tE[elim!]: "\<turnstile>UPTO\<lparr> (x ::= a,s,stack) \<Rightarrow>\<^bsup>p \<^esup> (t,b)"
inductive_cases uptoSeq_tE[elim!]: "\<turnstile>UPTO\<lparr> (c1;;c2,s1,stack1) \<Rightarrow>\<^bsup>p \<^esup> (t,b)"
inductive_cases uptoIf_tE[elim!]: "\<turnstile>UPTO\<lparr> (IF b \<noteq>0 THEN c1 ELSE c2,s,stack) \<Rightarrow>\<^bsup>x \<^esup> (t,CONT)"
inductive_cases uptoCall_tE[elim!]: "\<turnstile>UPTO\<lparr> (CALL C RETURN v,s,stack) \<Rightarrow>\<^bsup>z \<^esup> (t,ret)"
inductive_cases uptoRec_tE[elim]: "\<turnstile>UPTO\<lparr> (TAIL,s,stack) \<Rightarrow>\<^bsup>x \<^esup> (t,ret)"

lemma upto_rec_noninterference:
  "\<lbrakk>\<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup>t\<^esup> (s',stack',CONT); set (vars c) \<subseteq> S; v \<notin> S\<rbrakk> \<Longrightarrow> \<turnstile>UPTO\<lparr> (c,s(v:=y),stack) \<Rightarrow>\<^bsup>t\<^esup> (s'(v:=y),stack',CONT)"
proof (induction c s stack t s' stack' CONT rule: upto_trec_induct)
  case (Skip s stack)
  then show ?case 
    by blast
next
  ...
  thus ?case using upto_trec.Assign[of  x a "s(v:=y)" ret] by argo
next
  case (Call C s z t r ret)
  hence Call: "(C, s(v := y)) \<Rightarrow>\<^bsup>z \<^esup> t(v := y)" using fresh_var_changed by fastforce
  from Call have state: " s(v := y, r := (t(v := y)) r) = s(r := t r, v := y)" by auto
  show ?case using upto_trec.Call[OF Call, of  r] state by metis
next
  case (Rec s ret1)
...
lemma upto_rec_deterministic:
  "\<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup> t \<^esup> (s',stack',b) \<Longrightarrow> \<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup> t' \<^esup> (s'',stack'',b') \<Longrightarrow> t = t' \<and> s' = s'' \<and> stack' = stack'' \<and> b = b'"
proof (induction c s stack t s' stack' b arbitrary: t' s'' stack'' b' rule: upto_trec_induct)
  case (IfTrue s b c1 ret x t CONT y c2)
  then show ?case
    by fastforce
next
  case (IfFalse s b c2 stack x s' stack' CONT y c1)
  from \<open>s b = 0\<close> \<open>\<turnstile>UPTO\<lparr> (tsIf b c1 c2, s, stack) \<Rightarrow>\<^bsup>t'\<^esup>  (s'',stack'',b')\<close> obtain x' where
     \<open>\<turnstile>UPTO\<lparr> (c2, s, stack) \<Rightarrow>\<^bsup>x'\<^esup> (s'',stack'',b')\<close> and \<open>t' = x' + 1\<close>
    by auto
  hence \<open>x = x' \<and> s' = s'' \<and> stack' = stack'' \<and> CONT = b'\<close>
    using IfFalse.IH by blast
  then show ?case 
    by (simp add: \<open>t' = x' + 1\<close> IfFalse.hyps(3))
next
  case (Call C s z t r stack)
  then show ?case using bigstep_det by blast
qed blast+

lemma upto_no_rec: "\<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup>t\<^esup> (s',stack',b) \<Longrightarrow> \<not>b \<Longrightarrow> c' \<turnstile> (c,s,stack) \<Rightarrow>\<^bsup> t \<^esup> (s',stack')"
  by (induction c s stack t s' stack' b rule: upto_trec_induct) auto

lemma upto_false_no_rec: "\<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup>t\<^esup> (s',stack',b) \<Longrightarrow> \<not>stails c \<Longrightarrow> \<not>b"
  by (induction c s stack t s' stack' b rule: upto_trec_induct) auto

lemma upto_no_rec_complete': "c' \<turnstile> (c,s,stack) \<Rightarrow>\<^bsup> t \<^esup> (s',stack') \<Longrightarrow> \<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup> t' \<^esup> (s'',stack'',False)\<Longrightarrow> s' = s'' \<and> stack' = stack'' \<and> t = t'"
  apply (induction c' c s stack t s' stack' arbitrary: t' s'' stack'' rule: tsbig_step_t_induct) 
  apply auto
  using bigstep_det apply blast
  using bigstep_det apply blast
       apply (metis tsPush ts_deterministic upto_no_rec)
      apply (metis tsPush ts_deterministic upto_no_rec)
      apply (metis tsPush ts_deterministic upto_no_rec)
      apply (metis tsPop ts_deterministic upto_no_rec)
      apply (metis tsPop ts_deterministic upto_no_rec)
  by (metis tsPop ts_deterministic upto_no_rec)

lemma upto_no_rec_sound: "\<turnstile>UPTO\<lparr> (c,s,stack) \<Rightarrow>\<^bsup> t \<^esup> (s',stack',b) \<Longrightarrow> \<not> stails c \<Longrightarrow> c' \<turnstile> (c,s,stack) \<Rightarrow>\<^bsup> t \<^esup> (s',stack')"
