\<^marker>\<open>contributor "Ala Sleimi"\<close>
...
text \<open>Reduction from "Karp, R. M. (1972). Reducibility among combinatorial problems"\<close>
...
theorem not_reduction_XC_to_steiner_tree:
  shows "\<not>is_reduction (XC_to_steiner_tree :: nat set \<times> _  \<Rightarrow> _) exact_cover steiner_tree"
  using XC_to_steiner_tree_counterexample unfolding is_reduction_def by fast
