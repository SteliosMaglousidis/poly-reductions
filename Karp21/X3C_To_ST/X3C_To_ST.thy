...
lemma disjoint_if_sum_card_eq_card:
  assumes "finite (\<Union>S)" "sum card S = card (\<Union>S)"
  shows "disjoint S"
...
proof -
  from assms obtain S' where
    ...
    unfolding three_exact_cover_def exact_cover_def by auto
...
proof (cases "\<forall>C\<in>S. card C = 3")
  case True
   ...
   show ?thesis
   proof (intro three_exact_coverI exact_coverI[of S'])
     show "finite X"
     ...
     then have "disjoint S'" using disjoint_if_sum_card_eq_card using \<open>\<Union> S' = X\<close> \<open>finite X\<close> by blast
     with \<open>\<Union> S' = X\<close> show "cover S' X" by (intro coverI) auto
  qed
  next
    case False
