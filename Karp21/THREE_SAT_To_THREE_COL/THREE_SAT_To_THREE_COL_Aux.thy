theory THREE_SAT_To_THREE_COL_Aux
...
subsection \<open>CNF Aux for THREE_SAT_To_THREE_COL\<close>


lemma cnf_remove_clause: "F \<in> three_sat \<Longrightarrow> F = F' @ [a] \<Longrightarrow> F' \<in> three_sat"
proof -
  assume asm1: "F\<in> three_sat"
  assume asm2: "F = F' @ [a]"
  from asm1 obtain \<sigma> where obt1:"\<sigma> \<Turnstile> F " unfolding three_sat_def sat_def by auto
  from asm2 have "(set F') \<subseteq> (set F)" by auto
  from this obt1 have "\<sigma> \<Turnstile> F'" unfolding models_def
    by blast
  from asm1 asm2 this show ?thesis by force
qed


lemma cnf_clause: "F \<in> three_sat \<Longrightarrow> F = F' @ [a] \<Longrightarrow> [a] \<in> three_sat"
proof -
  assume asm1: "F\<in> three_sat"
  assume asm2: "F = F' @ [a]"
  from asm1 obtain \<sigma> where obt1:"\<sigma> \<Turnstile> F " unfolding three_sat_def sat_def by auto
  ...
  from this obt1 have "\<sigma> \<Turnstile> [a]" unfolding models_def
    by blast
  from asm1 asm2 this show ?thesis by fastforce
qed

subsection \<open>Graph Aux for THREE_SAT_To_THREE_COL\<close>
...
lemma finite1:
"F\<in> three_sat \<Longrightarrow>
finite {{(0, False, 0, 2), (Suc 0, True, var l, 0)} |j l. l \<in>  \<Union> (set F)} "
proof -
  assume "F\<in> three_sat"
  from this have "finite (\<Union> (set F))" unfolding three_sat_def sat_def models_def
