theory THREE_SAT_To_THREE_COL
  imports THREE_SAT_To_THREE_COL_Aux
begin
...
definition
  "sat_three_col F \<equiv> if (\<forall>cls \<in> set F. card cls = 3) then (
...
lemma sat_three_col_ugraph: "F \<in> three_sat \<Longrightarrow> ugraph (sat_three_col F)"
  unfolding ugraph_def sat_three_col_def three_sat_def
  using finite1 finite2 finite3 finite4 unfolding three_sat_def
  by auto
...
lemma sat_to_three_color: "F \<in> three_sat \<Longrightarrow> sat_three_col F = E \<Longrightarrow> \<sigma> \<Turnstile> F \<Longrightarrow>
  ...
  from F_empty 0 have E_def_empty:
                          "E = {{false_node,true_node},
                               ...
                               {false_node,help_node}}"
                                unfolding sat_three_col_def by auto
...
next
  case (Suc x)
    ...
    (*--------------------------------------*)
    from Suc(3) Suc(4) sat_three_col_ugraph have E_ugraph:"ugraph E"
      by blast
    from Suc(2) obtain F' a where F'_def:"F' @ [a]= F "
      by (metis append.right_neutral append_eq_conv_conj id_take_nth_drop lessI)
    from this Suc(3) have F'cnf:"F' \<in> three_sat"
      using cnf_remove_clause by auto
    from F'_def have F'_def_rev: "F = F' @ [a]"
      by simp
    from F'_def Suc(3) have a_card:"card a = 3" unfolding three_sat_def by fastforce
    ...
    define E' where E'_def:"E' = sat_three_col F'"
    from Suc(5) F'_def Suc(3) have  ass:"\<sigma> \<Turnstile> F'"
      unfolding three_sat_def sat_def
    ...
    proof -
      from Suc(3) Suc(4) have E_def_graph:"E = {{false_node, true_node}, {true_node, help_node},
          ...
          {{(2, False, i, 5), false_node} |i. i < length F} \<union>
          {{(2, False, i, 5), help_node} |i. i < length F} "
        unfolding three_sat_def sat_three_col_def by force
      ...
      from E'_def F'cnf have E'_def_graph:"E' =
          ...
          {{(2, False, i, 5), false_node} |i. i < length F'} \<union>
          {{(2, False, i, 5), help_node} |i. i < length F'} "
        unfolding three_sat_def sat_three_col_def by auto
    ...
    from F'cnf have E_diff_v_main_subset:"E_diff_v_main \<subseteq> \<Union> E'"
      unfolding sat_three_col_def three_sat_def E_diff_v_main_def E'_def
      by force
    ...
    proof -
      ...
      from F'cnf have  E'_split:
        ...
        unfolding sat_three_col_def three_sat_def E'_def E'_main_def E'_var_def E'_gadget_def
        by auto
    ...
    from E'_def F'cnf F'_length have newGadgetNotInE':"\<forall>y. \<forall>z. (2::nat,y,x,z) \<notin> \<Union> E'"
      unfolding sat_three_col_def by fastforce
    ...
    from E'_def F'cnf have var_NotIn_E':"\<And>lit .( var lit \<notin> var ` \<Union> (set F') \<Longrightarrow>
      ...
      unfolding sat_three_col_def three_sat_def
    proof -
      fix l
    ...
    proof -
      ...
      next
        case False
        from this asm1 show ?thesis
          by (metis lit.simps(5) models_lit_def lit.simps(6) tolit.elims var.simps(2))
      qed
    qed
    ...
    proof -
      ...
      next
        case False
        from this asm1 show ?thesis unfolding models_lit_def
    ...
    proof -
      ...
      next
        ...
        (tolit l1 \<noteq> Pos \<and> var l1 = var l \<longrightarrow> tolit l2 = Pos \<longrightarrow> var l2 \<noteq> var l)"
          unfolding models_lit_def by (smt lit.simps(5) lit.simps(6) var.elims)
        then show ?thesis
          by blast
      qed
    qed
...
lemma sat_three_col_card3:"sat_three_col F \<in> three_colorability \<Longrightarrow> \<forall>cls\<in>set F. card cls = 3"
  ...
  next
    case False
    assume asms: "sat_three_col F \<in> three_colorability"
    from False have "sat_three_col F = {{(0,False,0,0)}}" unfolding sat_three_col_def
      by presburger
    from this have "\<not> ugraph (sat_three_col F)" unfolding ugraph_def
      by simp
...
lemma sat_three_col_models:"sat_three_col F \<in> three_colorability \<Longrightarrow> \<exists>\<sigma>. \<sigma> \<Turnstile> F"
proof (cases F)
  case Nil
...
next
  ...
  from this have F_cls_card_3:"\<forall>cls\<in>set F. card cls = 3"
    using sat_three_col_card3
    by presburger
  define E where E_sat_three_col_def:"E = sat_three_col F"
  ...
  from E_sat_three_col_def F_cls_card_3 have E_def: "E =
  ...
  proof-
    ...
    proof (cases "tolit l = Pos")
      ...
      from asms this have "(1, True, var l,0) \<in> c1"
        by simp
      from this True \<sigma>_def show ?thesis unfolding models_lit_def
    ...
    next
      ...
      from this l_F_T_edge_in_E c1_iscolor asms(2) have "(1, True, var l,0) \<notin> c1"
        by blast
      from this asms \<sigma>_def l_is_neg show ?thesis unfolding models_lit_def
        by auto
    qed
  qed
...
theorem is_reduction_sat_three_col:
  "is_reduction sat_three_col three_sat three_colorability"
  unfolding is_reduction_def
proof safe
  fix F :: "nat lit set list"
  assume 1:"F \<in> three_sat"
  then obtain \<sigma> where ass:"\<sigma> \<Turnstile> F"
    unfolding three_sat_def sat_def by auto
  from \<open>F \<in> _\<close> have fin_1: "finite (\<Union> (set F))"
    unfolding three_sat_def by (force intro: card_ge_0_finite)
  obtain E where E_def:"sat_three_col F = E"
    by simp
  from this 1 have graph:"ugraph E" using sat_three_col_ugraph
    by blast
  from E_def 1 ass have "E\<in> three_colorability"  using sat_to_three_color[where F = F]
    unfolding three_colorability_def by fastforce
  from this E_def show "sat_three_col F \<in> three_colorability" by auto
next
  fix F :: "nat lit set list"
  obtain E where E_def:"sat_three_col F = E"
    by simp
  assume E_3c:"sat_three_col F \<in> three_colorability"
  ...
  from E_3c have "\<exists>\<sigma>. \<sigma> \<Turnstile> F "
    using sat_three_col_models
    by simp

  from this F_3_card show "F \<in> three_sat" unfolding three_sat_def sat_def
    by blast
qed
