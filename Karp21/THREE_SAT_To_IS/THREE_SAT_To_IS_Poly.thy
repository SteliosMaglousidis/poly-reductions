...
definition "size_IS \<equiv> (\<lambda>(E,k). card E)"
...
lemma sat_to_is_size: "size_IS (sat_is p) \<le> sat_to_is_space (size_SAT p)"
  apply(auto simp: size_IS_def sat_is_def sat_to_is_space_def size_SAT_def
    sat_is_un_1_def sat_is_un_2_def)
  ...
  subgoal
    ...
    subgoal for i apply simp
      apply(rule upperbounding_card3) by (auto simp: is_n_sat_def)
    apply simp
    done
  ...
  subgoal
    ...
    apply(rule order.trans)
      ...
      subgoal
        apply simp apply(rule upperbounding_card3) by (auto simp: is_n_sat_def)
      apply simp done
...
theorem sat_to_is_ispolyred: "ispolyred sat_to_is three_sat independent_set size_SAT size_IS"
