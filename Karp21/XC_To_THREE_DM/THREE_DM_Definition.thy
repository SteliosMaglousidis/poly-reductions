...
definition "distinct_components3 M1 M2 \<equiv> \<forall>(a, b, c) \<in> M1.  \<forall>(a1, b1, c1) \<in> M2.
...
lemma self_distinct_components3_injI:
  assumes "inj_on f S" "inj_on g S" "inj_on h S"
...
lemma self_distinct_components3_unionI:
  assumes "self_distinct_components3 M1" "self_distinct_components3 M2" "distinct_components3 M1 M2"
...
lemma self_distinct_components3_subset:
  assumes "self_distinct_components3 B" "A \<subseteq> B"
...
lemma distinct_components3I:
  assumes "\<And> a b c a1 b1 c1.  (a,b,c) \<in> M1 \<Longrightarrow> (a1,b1,c1) \<in> M2 \<Longrightarrow> (a=a1 \<or> b=b1 \<or> c=c1) \<Longrightarrow>
    ((a,b,c) = (a1,b1,c1))"
...
lemma three_dm_cert:
  ...
  shows "(U,T) \<in> three_dm"
  unfolding three_dm_def using assms
  by blast
...
definition "three_dm_alt1 \<equiv>
...
lemma three_dm_to_alt1_sound:
  assumes "(U, T) \<in> three_dm"
  shows "(three_dm_to_alt1 (U,T)) \<in> three_dm_alt1"
proof -
  obtain M where "M \<subseteq> U"  "card M = card T" "self_distinct_components3 M"
    using assms unfolding three_dm_def
    by blast
...
proof -
  ...
  ultimately show ?thesis
    using assms \<open>card M' = card T\<close>
    unfolding three_dm_def three_dm_alt1_def three_dm_to_alt1_def Let_def
    [[simproc add: finite_Collect]]
    by (auto simp add: inj_on_def finite_image_iff, blast+)
qed

theorem is_reduction_three_dm_to_alt1:
  "is_reduction three_dm_to_alt1 three_dm three_dm_alt1"
  using three_dm_to_alt1_sound three_dm_to_alt1_complete
  by (intro is_reductionI) auto

text \<open>Example from Wikipedia:
https://en.wikipedia.org/wiki/3-dimensional_matching#Decision_problem\<close>

definition "three_dm_alt2 \<equiv>
    {(T, k). finite T  \<and>
...
proof -
  obtain M where "M \<subseteq> T"  "card M \<ge> k"  "self_distinct_components3 M"
    using assms unfolding three_dm_alt2_def by auto
...
theorem is_reduction_three_dm_to_alt2:
  "is_reduction three_dm_to_alt2 three_dm three_dm_alt2"
  using infinite_super alt2_exact_answer
  unfolding is_reduction_def three_dm_alt2_def three_dm_to_alt2_def three_dm_def
  by (auto, blast)
