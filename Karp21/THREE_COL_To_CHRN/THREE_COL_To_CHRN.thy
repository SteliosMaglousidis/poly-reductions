theory THREE_COL_To_CHRN
  imports THREE_COL_To_CHRN_Aux
begin

subsection "Chromatic number definitions"

definition "chromatic_number \<equiv> {(E,k). \<exists>c_Sets. is_k_colorable E k c_Sets \<and> k \<ge> 3}"

definition
  "three_col_to_chrn E \<equiv> if (ugraph E) then ((
...
lemma three_col_to_chrn_edge_in_E_iff:
  "ugraph E \<Longrightarrow> {v1,v2} \<in> E  \<longleftrightarrow> {(v1::nat,0::nat),(v2,0)} \<in> fst(three_col_to_chrn E)"
proof safe
  assume asms: "ugraph E" "{v1, v2} \<in> E "
  then show "{(v1, 0), (v2, 0)} \<in> fst (three_col_to_chrn E)"
    unfolding three_col_to_chrn_def
    by fastforce
next
  fix E v1 v2
  assume asms: "ugraph E" "{(v1, 0), (v2, 0)} \<in> fst (three_col_to_chrn E)"
  obtain E' where E'_def2:"E' = fst (three_col_to_chrn E)"
    by blast
...
lemma nodesChromGraph:
    "(E',k) = three_col_to_chrn E \<Longrightarrow> ugraph E \<Longrightarrow> \<Union>E' = \<Union>{{(v,0)}|v. v\<in>\<Union>E} \<union> \<Union>{{(0,n)|n. n\<in>{1..card (\<Union>E)}}}"
proof -
  fix E E' k
  assume asms:"(E',k) = three_col_to_chrn E" "ugraph E"
...
lemma threeColToChroN:"\<And>E. E \<in> three_colorability \<Longrightarrow> three_col_to_chrn E \<in> chromatic_number"
proof -
  fix E :: "nat set set"
  ...
  from asms have E_ugraph:"ugraph E"
    ...
    by blast
  obtain E' k where E'_def:"(E', k) = three_col_to_chrn E"
    by (metis surj_pair)
  ...
  proof -
    from E'_def2 E_ugraph have "finite E'"
      ...
      by (smt E'_def E_ugraph card_eq_0_iff finite_UnI finite_Union
          prod.inject three_col_to_chrn_def zero_neq_numeral)
...
from this have c2'c3'_noteq:"\<Union>{{(v,0::nat)}|v. v:c2} \<noteq> \<Union>{{(v,0::nat)}|v. v:c3}"
  ...
  from E'_def have "(E', k) \<in> chromatic_number"
    unfolding chromatic_number_def is_k_colorable_def is_colorable_def
    using c_Sets'_complete E'_ugraph color_disjoint c_Sets_sound k_card kgreaterThr
    by blast
  from this E'_def show "three_col_to_chrn E \<in> chromatic_number"
    by simp
qed


lemma chroNToThreeCol_ugraph:"\<And>E. ugraph E \<Longrightarrow> three_col_to_chrn E \<in> chromatic_number \<Longrightarrow> E \<in> three_colorability"
proof -
  fix E :: "nat set set"
  assume asms:"three_col_to_chrn E \<in> chromatic_number" "ugraph E"
  obtain E' k  where E'def_2:"(E',k)=three_col_to_chrn E"
    by (metis surj_pair)
  from this asms have E'chrN:"(E',k) \<in> chromatic_number"
    by simp
  from E'def_2 have E'_def_3:"fst (three_col_to_chrn E) = E'"
    by (metis fstI)
  ...
  proof -
    ...
    from this asms(2) E'_def_3 show "{v,v'}\<notin> E"
      using three_col_to_chrn_edge_in_E_iff
      by blast
  qed
...
lemma chroNToThreeCol_notugraph:"\<And>E. \<not>ugraph E \<Longrightarrow> three_col_to_chrn E \<in> chromatic_number \<Longrightarrow> E \<in> three_colorability"
proof -
  fix E :: "nat set set"
  assume asms:"three_col_to_chrn E \<in> chromatic_number" "\<not> ugraph E"
  obtain E' k  where E'_def:"(E',k) =three_col_to_chrn E"
    by (metis surj_pair)
  from this asms have E'_chrN:"(E',k) \<in> chromatic_number"
    by simp
  from asms E'_def have "k = (0::nat)"
    unfolding three_col_to_chrn_def
    by auto
  from this E'_chrN have "(E',0::nat) \<in> chromatic_number"
    by auto
...
lemma chroNToThreeCol:"\<And>E. three_col_to_chrn E \<in> chromatic_number \<Longrightarrow> E \<in> three_colorability"
proof -
  fix E :: "nat set set"
  assume asms:"three_col_to_chrn E \<in> chromatic_number"
...
theorem is_reduction_three_col_chrn:
  "is_reduction three_col_to_chrn three_colorability chromatic_number"
