...
proof -
  ...
  using assms  unfolding three_dm_def by auto
  ...
  then have "(a ` T \<union> b ` T \<union> c ` T, {{a x, b y, c z} |x y z. (x, y, z) \<in> U}) \<in> three_exact_cover"
    using M_def S'_def unfolding distinct_components3_def
    by (intro three_exact_coverI exact_coverI[of S'])
    (fastforce, auto simp add: disjoint_def cover_def)
  then show ?thesis unfolding three_dm_to_x3c_def by fast
qed
...
proof -
  ...
  from assms obtain S' where
    ...
    unfolding three_exact_cover_def three_dm_to_x3c_def S_def exact_cover_def by auto
...
theorem is_reduction_three_dm_to_x3c: "is_reduction three_dm_to_x3c three_dm three_exact_cover"
  using three_dm_to_x3c_sound three_dm_to_x3c_complete by (intro is_reductionI) auto
