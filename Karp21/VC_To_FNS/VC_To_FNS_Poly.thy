...
lemma e_in_E_e_explicit:
  ...
  using assms card_2_iff by force

lemma card_verts_ugraph:
  assumes "ugraph E"
  shows "card (\<Union> E) \<le> 2 * card E"
  using assms by (simp add: card_union_if_all_subsets_card_i ugraph_def)

lemma card_verts:
  assumes "ugraph E"
  shows "card {v. \<exists>x\<in>E. v \<in> x} \<le> 2 * (card E)"
  by (metis Union_eq assms card_verts_ugraph)

lemma card_arcs:
  assumes "ugraph E"
  shows "card {(u, v). {u, v} \<in> E} \<le> 2 * card E" (is "card ?S \<le> _")
  using assms by (intro card_ordered_pairs) auto
